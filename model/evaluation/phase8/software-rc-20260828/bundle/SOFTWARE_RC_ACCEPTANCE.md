...
- `npm run security:scan`：300 个文本文件、118 个公开证据文件，0 secret、0 hidden-field，HMAC 配置完整。
...
- 历史废弃评测批次记录了第三方端点间歇性 unavailable；最终 RC 使用 15 秒会话冷却后 25/25 通过。生产部署仍需在目标服务器验证真实并发与限流配置。
