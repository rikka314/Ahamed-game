...
> 当前基线：Node.js + TypeScript 独立 package，Phase 8-A 已完成，状态为 **Software RC ready；Formal MVP blocked by infrastructure**。唯一批准的第三方 OpenAI-compatible Provider 固定使用 `gpt-5.6-sol`；候选 15/15、最终 RC 25/25，Controller fact-routing 100%，60/100 条患者回复双角色审核 approved。runtime-release manifest 已验证，share 保留 `v1-rc1`。Claude 未进入批准列表；Phase 8-B 仍需已知目标服务器 smoke。
...
CLI 默认将 SQLite 写入 `model/var/model-cli.sqlite`，也可用 `--database` 或 `AHAMED_MODEL_DATABASE_PATH` 指定。交互式 CLI 当前只开放固定 `deterministic-v1`；OpenAI 与 Anthropic 仅用于各自的固定 C01 live-eval 命令。在剩余 Phase 7 端到端候选评测与运行时发布门通过前，远程 Provider 不接收任意交互输入，也不会在 session 中途自动切换 Provider。人工审核或签字不再是该发布门的组成部分。

任何使用 SQLite 的 CLI/服务或 `ops:recover` 启动都必须由运行环境注入至少 32 字符的 `SAFETY_AUDIT_HMAC_KEY`。该密钥必须跨重启保持稳定，用于安全退出与 turn 幂等 fingerprint、待 Controller 判定文本的 AES-256-GCM 域分离密钥，以及 `response_validated` buffer 的服务端 HMAC 完整性标签；不得提交到仓库。`ModelPersistence.requiresStableIntegrityKey` 会阻止公开低级构造方式绕过该要求。现阶段如需轮换密钥，必须先完成显式数据迁移或清理对应开发数据库，不能在已有持久化数据上静默更换。
...
Anthropic adapter 固定到官方 `https://api.anthropic.com`，忽略 `ANTHROPIC_BASE_URL`，并关闭 SDK 自动重试；应用层仍使用同一份 Controller/Patient/Review 角色端口与业务 Schema。它保持为离线验证通过的可选 adapter，不会读取或复用当前第三方 `MODEL_API_KEY`，也不会进入当前 MVP session。若未来重新启用真实 C01，必须另行批准并配置独立 `ANTHROPIC_API_KEY`。
...
- SQLite 启用 foreign keys、WAL、`synchronous=FULL` 和 busy timeout；当前 Schema migration 为 v5，可从 v1/v2/v3/v4 原位升级。会话、回合、Controller 分类、请求 fact IDs、medical/repeat/other 计数、持久化 turn-attempt 成本预算、已披露事实、检查、诊断、评估、幂等记录、操作日志与只追加审计事件在本地事务内原子提交；事实和检查使用显式 ordinal 保持重启前后的公开投影顺序。
...
- 新 turn 的 request hash 从第一笔写入起就是服务端 HMAC；待 Controller 判定的文本使用 AES-256-GCM 加密，随机 96-bit IV，AAD 绑定 operation/session/idempotency/HMAC、Provider/model/prompt/case、client ID 和长度。执行与显式恢复只在内存中解密；Controller 放行后才允许进入 validated buffer 和正常病例回合，`unsafe` 则清除密文并只保留长度/HMAC。因此 SQLite 主文件、WAL 和 SHM 不会出现 provider-fallback 原文。
...
- `src/cli/command-parser.ts` 固定 `/cases`、`/start`、`/resume`、`/status`、问诊、检查、鉴别诊断、诊断、结果、取消、帮助和退出命令；普通自然语言只进入问诊，不会隐式申请检查或提交诊断。
...
- `ControllerProvider`、`PatientProvider`、`ReviewProvider` 是三个独立角色端口；`ModelProvider` 继续作为 `ModelService` 的兼容 façade，避免破坏 deterministic、恢复和持久化边界。
...
- `OpenAIModelProvider` 使用官方 `openai@7.7.0` Responses SDK；默认固定官方端点，也支持通过 `MODEL_BASE_URL + MODEL_API_KEY` 显式选择第三方 Responses-compatible HTTPS 端点。端点内容只以确定性 SHA-256 假名标识进入 Provider identity、审计和 benchmark fingerprint，不记录原始 URL；该 hash 不是匿名化结果，已知候选 URL 仍可枚举。官方 Key 永不转发给第三方，第三方 Key 也不会回退到官方端点。三个角色分别使用 strict JSON Schema Structured Outputs，明确设置 `store=false`；SDK 自动重试关闭，首次持久化 operation 在 Controller/Patient/Review 间共享至多一次自动重试，显式恢复不会重置这笔预算。
- Controller 只发送问题、locale 和可问 fact 索引；Patient 只发送当前问题、本轮 allowed facts、公开语言风格和已披露事实；Review 只发送五个确定性分项、固定公开 evidence 投影、已披露轨迹/已完成检查和审核后的沟通 criterion ID，不发送 target diagnosis、私有 evidence ID、must-ask fact、鉴别概念、检查分类、userId、完整 CasePackage 或 API Key。`review()` 在实际 Provider 边界会再次从数值分项重建 allowlist 投影，调用方不能借内部 DTO 透传私有证据。Patient 模型原始自然语言不会提交给 CLI，最终话术由服务端从模型选择的已授权病例事实确定性生成；`unknown` 只生成不确定表述。
- Controller 的 medical/repeat/other 分类和 requested fact IDs 会持久化；repeat 和 other 分别进入效率扣分，other 不占 medical turn quota，空的 `ask_patient` 映射 fail closed。每个新的 turn 幂等键在 Provider 前持久化消耗一次预算，成功、Provider 失败和输出拒绝都计入，幂等重放不重复计数；单会话达到 25 次后在下一次 Provider 调用前拒绝。
...
- 当前 C01–C05 已达到 5/5 structurally ready、5/5 publishable，并已物化为 5 个 published 病例。manifest 按 `publicCaseId + caseVersion + contentHash + releaseValidationMethod=ai_cross_validation` 冻结发布集；`npm run cases:validate` 会在绑定漂移、重复验证器、角色缺失或任一检查拒绝时 fail closed。
...
- 同一稳定 HMAC 密钥下，本地安全退出可在 SQLite 重启后以同一幂等键重放且不重复调用 Provider/写事件；`response_validated` 的 turn/evaluation 缓冲同时验证 checksum、服务端 HMAC、不可变原请求、当前病例事实和本地评分，任何重算普通 hash、改写 validatedAt、替换问诊文本或评分内容的篡改都会 fail closed。Provider Controller 的 `unsafe` 仅作为本地规则漏检后的纵深防线：待分类文本在首笔持久化前已变为 AEAD 密文，因此此分支即使产生 1 次 Controller 调用，SQLite/WAL/SHM 也没有原文；随后清除密文、只保留长度/HMAC，以同一幂等键稳定重放，且不进入 Patient/Evaluator。
...
- 100 条病例问法已逐条穿过真实 `ModelService` 安全前置门并进入 Controller；32 条对抗输入已逐条经真实 `ModelService.askPatient` 本地拒绝，均为 0 Provider 调用、0 operation、0 回合和 0 原文事件写入。
...
- manifest 当前包含 5 个 AI 交叉验证的 published 病例，因此完整候选 benchmark 返回 `FULL_CANDIDATE_BENCHMARK_READY`。该状态只允许开始真实候选评测；不得用 mock 或既有只加载 C01 reference draft 的单例 `eval:live` 代替 5 个 published 病例重复评测，也不得据此宣称 H8 或 Software RC 已通过。
...
- 本地安全前置退出不会创建 operation、调用 Provider、增加病例回合或保存原始输入；只记录服务端密钥 HMAC-SHA-256 和版本化审计元数据。所有待 Controller 判定的 turn 文本从首笔持久化开始就是 AEAD 密文、request hash 从首笔写入开始就是 HMAC；若纵深 Controller 返回 `unsafe`，只允许已有的 Controller 调用，随后清除密文、只保留长度/HMAC，且不得进入 Patient/Evaluator。`SAFETY_AUDIT_HMAC_KEY` 在部署时必须由 secret manager 注入，不能写入仓库。
...
- Phase 7、P8-PRE-01–P8-PRE-05 和 Phase 8-A：**已完成**。最终 Software RC 证据位于 `evaluation/phase8/`；验收报告与可复制 zip 位于 `evaluation/phase8/software-rc-20260828/`。
...
- Phase 8-B：**blocked by infrastructure**。必须使用同一 Software RC zip 在目标服务器验证单进程、SQLite/WAL/备份恢复、secret 注入、时钟/TTL、四阶段崩溃恢复和 Provider session 连通后，才能称正式 MVP。
