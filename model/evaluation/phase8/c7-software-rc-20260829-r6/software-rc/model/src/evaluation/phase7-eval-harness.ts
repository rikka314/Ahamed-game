import {
  ...
  PHASE7_CASE_STATUS,
  PHASE7_DRAFT_ASKABLE_FACT_IDS_BY_CASE,
  PHASE7_EVAL_CORPUS,
...
export function validatePhase7EvalCorpus(
  ...
  if (corpus.caseCorpora.length !== 5) {
    issues.push(`corpus must contain exactly 5 cases; found ${corpus.caseCorpora.length}`);
  }
  ...
  for (const caseCorpus of corpus.caseCorpora) {
    ...
    const expectedAllowlist =
      PHASE7_DRAFT_ASKABLE_FACT_IDS_BY_CASE[caseCorpus.caseId];
    if (
      expectedAllowlist === undefined ||
      expectedAllowlist.length !== askableFactIds.size ||
      expectedAllowlist.some((factId) => !askableFactIds.has(factId))
    ) {
      issues.push(`case ${caseCorpus.caseId} askableFactIds must match its draft allowlist`);
    }
...
export function runPhase7OfflineEvalHarness(options: {
  ...
  const blockers = !validation.valid
    ...
    : publicationBlocked
      ? [
          `完整候选 benchmark 必须精确绑定评估语料的 5 个已发布病例；当前匹配 ${publishedCases} 个。`,
        ]
      : [];
