...
const draftFileByCaseId = new Map([
  ["case_c01_respiratory_001", "c01-common-cold-v1.json"],
  ["case_c02_respiratory_002", "c02-influenza-v1.json"],
  ["case_c03_respiratory_003", "c03-acute-pharyngitis-v1.json"],
  ["case_c04_respiratory_004", "c04-acute-bronchitis-v1.json"],
  ["case_c05_respiratory_005", "c05-mild-cap-v1.json"],
]);

test("Phase 7 corpus contains five development-only draft cases with exactly twenty Chinese items each", () => {
  assert.equal(PHASE7_EVAL_CORPUS.caseCorpora.length, 5);
  ...
  for (const caseCorpus of PHASE7_EVAL_CORPUS.caseCorpora) {
    ...
    const draftFile = draftFileByCaseId.get(caseCorpus.caseId);
    assert.ok(draftFile);
    const draft = JSON.parse(
      readFileSync(resolve("cases/draft", draftFile), "utf8"),
    ) as DraftCase;
