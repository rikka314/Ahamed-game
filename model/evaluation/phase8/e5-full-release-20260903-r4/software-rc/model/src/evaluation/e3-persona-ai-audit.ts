  ...
  return {
    ...
    validations: roles.map((role) => ({
      ...
      promptVersion: role === "persona_consistency_reviewer"
        ? "e3-persona-consistency-review-v1"
        : "e3-medical-fact-boundary-review-v1",
...
export async function generateE3PersonaAiCrossReview(input: {
  ...
  try {
    const response = await createAuditResponse(input.transport, {
      ...
      instructions:
        "你是隔离运行的 Persona Consistency Reviewer。根据六种人格目录，对六条匿名完整旅程逐一识别最符合的人格，并统计每条旅程中符合该人格且仍保持患者角色的轮数。只根据实际回复判断，不得假设 runId 暗示人格；你看不到 Medical Fact Boundary Reviewer 的任何输出。",
  ...
  try {
    ...
    for (let offset = 0; offset < input.samples.length; offset += batchSize) {
      ...
      const response = await createAuditResponse(input.transport, {
        ...
        modelId: input.modelId,
        instructions:
          "你是隔离运行的 Medical Fact Boundary Reviewer。逐条核对患者回复是否只使用 authorizedFacts、authorizedPersonaFacts、authorizedTestReports 与 authorizedTests；检查严重医学事实错误、标准诊断泄漏、未完成检查结果泄漏，以及把 unknown 事实说成明确没有或正常。你看不到 Persona Consistency Reviewer 的任何输出，不得把风格差异当作医学错误。",
        input: JSON.stringify({
