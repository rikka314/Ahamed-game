...
> 当前基线（2026-09-03）：可玩问诊只使用真实 OpenAI-compatible Patient Agent v0.5.0；每轮传入完整已提交对话历史。患者显示名仅是场景称呼或姓氏提示；医疗事实与显式年龄/性别锚点服从病例，未提供的姓名、家庭、住址和普通日常经历由 AI 合理补全并跨轮保持一致。患者不得用“病例没有说明”“资料未提供”等幕后话术回避。本地不使用固定问答、关键词路由、病例诊断词表或患者话术重渲染。Patient AI 在同一次调用中判断玩家是否已明确提交唯一主诊断：肯定的单一主诊断或清晰的“主诊断 + 鉴别诊断”会启动服务端评分，多病并列无主次、疑问和不确定讨论继续对话。AI 只提取玩家本轮原话中的诊断意图，不判断正确性；本地门禁校验提取词确实来自原话，最终正确性仍由确定性评分器判断。既有真实 `gpt-5.6-sol` 仅验证 v0.4 的三条诊断分支和非医疗角色历史一致性。E3 的 v0.5.0 六人格最新真实对照在 5 个已提交轮后因 Provider `OPENAI_TRANSIENT` 记录为 `not_run`，未形成质量通过证据。E5 已按当前 dirty 源码生成可复验 runtime manifest 与 Software RC；20/20 本地门禁通过，但内容和真实模型质量结论仍为 `incomplete`。旧 C6/C7 证据继续只作历史记录。
...
- E3 六人格专项 benchmark 已实现 72 条规则断言、固定 C01 医学锚点的六人格真实 runner（每人格 14 轮、目标共 84 轮）、逐问题不变量检查、14 轮虚构病例安全预检、两个隔离的非阻塞 AI 审阅角色、Windows 原子发布重试，以及规则语料/私有样本/审计/六份 journey 的完整 hash 绑定。当前 `evaluation/phase8/e3-persona-live-20260903-r7/` 因兼容 Provider 返回 `OPENAI_TRANSIENT`，v2 报告为 `not_run`：5 个已提交轮、2 个失败、4 个未运行、两个审核未运行；报告 SHA-256 为 `6f3c48cb9f26783be653c429f25666ba0a007cc943489c99c6bc7979ee16911f`。证据复验、工程链和 397/397 测试已通过，但不得把该记录解释为真实模型质量通过。
- E5 发布证据工程已实现。当前权威快照为 `evaluation/phase8/e5-full-release-20260903-r3/`：20/20 本地门禁通过、生产客户端 16 文件零敏感命中、RC 370 个文本文件及 4 份公开证据零命中、478 个文件 manifest 复验和四个外层 SHA-256 全通过；acceptance SHA-256 `f84d79e55d16b396bafb8758bb24a06191e8af1ba6b4c159db9cc8212fcd3757`，runtime manifest 文件 SHA-256 `0752645c222c30f27225237741a87c01099c62deed2cb09514584be1588ec637`、自哈希 `5e7830eade7a8024a0b9515973e575d5645a2226f87e39c05f316dcd5f9a54c3`，tar SHA-256 `e281463baca505abbe8b4e7bbbcb92f95621565e4e1c34600ee53e7f82d072d2`。decision 为 `incomplete`，因为 M0/B1–B5 未实施，实际仍为 5/30 病例、20/120 轨迹、30/180 黄金向量、100/600 样本、0/36 新域安全、0/60 当前病例 AI 审核、5/72 人格真实轮和 0/360 发布对话；E3/E4 最终真实观察为 `not_run`，旧审核为 `stale`。
- 当前批准范围只有第三方 OpenAI-compatible Provider + `gpt-5.6-sol`；Claude 真实验证延期且非阻塞，不存在自动 failover。
- 当前对话实现已完成 AI 主导/完整历史/诊断意图纠偏并通过全部测试与覆盖率门；v0.4 真实模型本地 smoke 已验证合理补全姓名、父母姓名、住址和昨日行程并保持一致，也验证了明确单诊断直接评分、清晰主诊断加鉴别诊断评分、多病无主次继续对话。E3 的 v0.5.0 真实运行当前为 `not_run`。旧 C6 r6、C7 runtime r10、dialogue-live r5、runtime manifest r11 和 Software RC r9 只作历史证据；E5 r3 已生成当前 dirty 源码快照，但 quality decision 为 `incomplete`。
