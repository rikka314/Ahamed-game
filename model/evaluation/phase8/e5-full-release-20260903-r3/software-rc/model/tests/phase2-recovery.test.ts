...
test("startup fails closed for a v3-shaped turn buffer without a validated Patient Agent output", async (t) => {
  ...
  delete encoded.payload["patientAgentOutput"];
...
test("startup fails closed for a v3 empty-fact buffer instead of synthesizing patient speech", async (t) => {
  const databasePath = await createDatabase(t);
  const seeded = seedSession(databasePath, "awaiting_model", "response_validated");
  assert.ok(seeded.operationId);
  const operationId = seeded.operationId;
