...
function isPublicEvidenceFile(gameRoot: string, path: string): boolean {
  const portable = relative(gameRoot, path).replaceAll("\\", "/");
  if (!portable.startsWith("model/evaluation/phase8/")) return false;
...
function main(): void {
  try {
    ...
    for (const path of files) {
      if (!isTextFile(path) || isIgnoredEnvironmentFile(gameRoot, path)) continue;
      scannedTextFiles += 1;
      ...
      if (isPublicEvidenceFile(gameRoot, path)) {
