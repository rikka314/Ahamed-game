...
- E3 六人格专项 benchmark 已实现 72 条规则断言、固定 C01 医学锚点的六人格真实 runner（每人格 14 轮、目标共 84 轮）、逐问题不变量检查、14 轮虚构病例安全预检、两个隔离的非阻塞 AI 审阅角色、Windows 原子发布重试，以及规则语料/私有样本/审计/六份 journey 的完整 hash 绑定。当前 `evaluation/phase8/e3-persona-live-20260903-r7/` 因兼容 Provider 返回 `OPENAI_TRANSIENT`，v2 报告为 `not_run`：5 个已提交轮、2 个失败、4 个未运行、两个审核未运行；报告 SHA-256 为 `6f3c48cb9f26783be653c429f25666ba0a007cc943489c99c6bc7979ee16911f`。证据复验、工程链和 409/409 测试已通过，但不得把该记录解释为真实模型质量通过。
- E5 发布证据工程已实现。当前权威快照为 `evaluation/phase8/e5-full-release-20260903-r6/`：20/20 本地门禁通过、生产客户端/预渲染 34 文件零敏感命中、RC 382 个文本文件及 4 份公开证据零命中、489 个文件 manifest 复验和四个外层 SHA-256 全通过；精确 SHA-256 以该目录的 `SHA256SUMS.json` 和 runtime manifest 自哈希为准。decision 为 `incomplete`，因为 M0/B1–B5 未实施，实际仍为 5/30 病例、20/120 轨迹、30/180 黄金向量、100/600 样本、0/36 新域安全、0/60 当前病例 AI 审核、5/72 人格真实轮和 0/360 发布对话；Provider、E3 与 E4 跨层/存储/双 AI 观察为 `not_run`，E3 复用及旧病例审核为 `stale`。r1–r5 仅保留为被 r6 取代的不可变审计记录。
- 当前批准范围只有第三方 OpenAI-compatible Provider + `gpt-5.6-sol`；Claude 真实验证延期且非阻塞，不存在自动 failover。
- 当前对话实现已完成 AI 主导/完整历史/诊断意图纠偏并通过全部测试与覆盖率门；v0.4 真实模型本地 smoke 已验证合理补全姓名、父母姓名、住址和昨日行程并保持一致，也验证了明确单诊断直接评分、清晰主诊断加鉴别诊断评分、多病无主次继续对话。E3 的 v0.5.0 真实运行当前为 `not_run`。旧 C6 r6、C7 runtime r10、dialogue-live r5、runtime manifest r11 和 Software RC r9 只作历史证据；E5 r6 已生成该 dirty 源码快照，但 quality decision 为 `incomplete`。
