...
const E4_REUSE_BINDING_SOURCE_PATHS = [
...
const E4_INDEPENDENT_AI_REVIEWER_IDS = [
...
function currentE4BindingStatus(gameRoot: string, record: unknown): E5EvidenceBinding["status"] {
  const reuseBindings = isRecord(record) && isRecord(record["reuseBindings"])
    ? record["reuseBindings"]
    : {};
  const files = Array.isArray(reuseBindings["files"]) ? reuseBindings["files"] : [];
  ...
  return reuseBindings["schemaVersion"] === "e4-reuse-bindings-v1" &&
      files.length === E4_REUSE_BINDING_SOURCE_PATHS.length &&
      fileBindings.size === E4_REUSE_BINDING_SOURCE_PATHS.length &&
      E4_REUSE_BINDING_SOURCE_PATHS.every((path) => {
        try {
          return fileBindings.get(path) ===
            sha256File(resolveContainedRegularFile(gameRoot, path, "E4 reuse binding"));
        } catch {
          return false;
        }
      })
...
function e4IndependentAiReviewStatuses(
  gameRoot: string,
  record: unknown,
): E5ObservationStatus[] {
  const reviewedContentSha256 = sha256Canonical(isRecord(record) ? {
    contractRelease: record["contractRelease"],
    engineeringStatus: record["engineeringStatus"],
    reuseBindings: record["reuseBindings"],
    metrics: record["metrics"],
    checks: record["checks"],
    verification: record["verification"],
  } : null);
  const reviews = arrayAt(record, "aiCrossReview", "reviews");
  if (reviews.length !== 2) return ["not_run", "not_run"];
  const declaredReviewerIds = new Set(
    reviews.flatMap((review) =>
      isRecord(review) && typeof review["reviewerId"] === "string"
        ? [review["reviewerId"]]
        : [],
    ),
  );
  if (
    declaredReviewerIds.size !== E4_INDEPENDENT_AI_REVIEWER_IDS.length ||
    E4_INDEPENDENT_AI_REVIEWER_IDS.some((reviewerId) => !declaredReviewerIds.has(reviewerId))
  ) {
    return ["stale", "stale"];
  }
  ...
  return reviews.map((review) => {
    ...
    const status = mapObservationStatus(review["status"]);
    if (status === "passed" || status === "failed") {
      if (
        typeof review["evidencePath"] !== "string" ||
        !review["evidencePath"].startsWith("versions/e4-ai-reviews/") ||
        evidencePaths.has(review["evidencePath"]) ||
        typeof review["evidenceSha256"] !== "string" ||
        !fileBindingMatches(
          resolve(gameRoot, "share"),
          review["evidencePath"],
      ...
      evidencePaths.add(review["evidencePath"]);
      try {
        const evidence = readJson(
          resolveContainedRegularFile(
            resolve(gameRoot, "share"),
            review["evidencePath"],
            "E4 independent AI review evidence",
          ),
        );
        return isRecord(evidence) &&
            evidence["schemaVersion"] === "e4-independent-ai-review-v1" &&
            evidence["reviewerId"] === review["reviewerId"] &&
            mapObservationStatus(evidence["decision"]) === status &&
            typeof evidence["modelId"] === "string" &&
            typeof evidence["promptVersion"] === "string" &&
            evidence["contentSha256"] === reviewedContentSha256
          ? status
          : "stale";
      } catch {
        return "stale";
...
function main(): void {
  ...
  try {
    ...
    if (
      commands.find(({ name }) => name === "model.eval:e3:persona-verify")?.exitCode !== 0
    ) {
      throw new Error("E3 evidence verification failed; E5 will not package unverified evidence.");
    }
    ...
    const manifestPath = "model/cases/manifest.phase6-compat.v2-rc1.json";
    const scoringTestPath = "model/tests/scoring-policy-v1.test.ts";
    ...
    const manifest = readJson(resolve(gameRoot, ...manifestPath.split("/")));
    ...
    const e3BindingStatus = currentE3BindingStatus(modelRoot, manifest, e3Report);
    const e4BindingStatus = currentE4BindingStatus(gameRoot, e4Record);
    const observedCounts: E5ObservedCounts = {
      cases: cases.length,
      trajectories: countRegressionTrajectories(modelRoot, manifest),
      goldenVectors: /freezes exactly 30 golden vectors/u.test(readFileSync(resolve(gameRoot, ...scoringTestPath.split("/")), "utf8")) ? 30 : 0,
    ...
    const evidenceBindings: E5EvidenceBinding[] = [
      ...
      collectEvidenceBinding(gameRoot, "current-case-manifest", manifestPath, "current"),
      collectEvidenceBinding(gameRoot, "scoring-golden-vector-test", scoringTestPath, "current"),
    ...
    const acceptanceReport = buildE5AcceptanceReport({
      ...
      e4: {
        ...
        aiReviews: e4IndependentAiReviewStatuses(gameRoot, e4Record),
      },
    ...
    const stagedManifest = readJson(
      resolve(releaseRoot, ...manifestPath.split("/")),
    );
    const stagedE3Report = readJson(
      resolve(releaseRoot, ...e3ReportPath.split("/")),
    );
    const stagedE4Record = readJson(
      resolve(releaseRoot, "share", "versions", "e4-patient-identity-quality-record.v1.json"),
    );
    if (currentE3BindingStatus(resolve(releaseRoot, "model"), stagedManifest, stagedE3Report) !== e3BindingStatus) {
      throw new Error("E3 freshness status changed during Software RC staging.");
    }
    if (currentE4BindingStatus(releaseRoot, stagedE4Record) !== e4BindingStatus) {
      throw new Error("E4 freshness status changed during Software RC staging.");
    }
