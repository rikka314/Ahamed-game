...
export function reconstructC7DialogueEvidence(input: {
  ...
  for (const journey of input.journeys) {
    ...
    if (journey.turns.length < input.policy.minimumRealDialogueTurnsPerCase) {
      throw new Error(
    ...
    const completedResult = resultTurn === undefined
      ? undefined
      : completedJourneyTest(resultTurn.sessionSnapshotBeforeTurn, chestCtTestId);
