...
> 当前基线（2026-09-04）：可玩问诊只使用真实 OpenAI-compatible Patient Agent v0.5.0；每轮传入完整已提交对话历史。患者显示名仅是场景称呼或姓氏提示；医疗事实与显式年龄/性别锚点服从病例，未提供的姓名、家庭、住址和普通日常经历由 AI 合理补全并跨轮保持一致。患者不得用“病例没有说明”“资料未提供”等幕后话术回避。本地不使用固定问答、关键词路由、病例诊断词表或患者话术重渲染。Patient AI 在同一次调用中判断玩家是否已明确提交唯一主诊断：肯定的单一主诊断或清晰的“主诊断 + 鉴别诊断”会启动服务端评分，多病并列无主次、疑问和不确定讨论继续对话。AI 只提取玩家本轮原话中的诊断意图，不判断正确性；本地门禁校验提取词确实来自原话，最终正确性仍由确定性评分器判断。M0 与 B1–B5 已形成 30 例、120 条轨迹、300 条黄金向量、600 条问诊样本及 60 次病例 AI 审核的完整内容集。E3 已记录 63 个真实锚点轮；C7 已覆盖 30 例和 360 个真实发布对话轮；E4 的 30 例跨层闭环、运行时存储扫描和两个隔离 AI 审核均完成。E5 最终候选位于 `evaluation/phase8/e5-full-release-20260904-r2/`，精确哈希以其中 `SHA256SUMS.json` 为准；工程与证据链完整，但质量 decision 为 `reported_with_failures`，不得解释为所有 AI 质量指标通过。旧 C6/C7/E5 证据继续只作历史记录。
...
- C01–C05 已迁移为 `CasePackage v2-rc1`，C06–C30 已按 M0/B1–B5 生产为 v2；活动清单 `cases/manifest.phase6-compat.v2-rc2.json` 精确绑定 30 个病例版本、content hash、人格、身份、provenance、红旗矩阵、回归和评测语料。
...
- 历史 C01–C05 v1 published 病例与 manifest 已被 C0 标记为 `superseded`。当前 30 例候选/发布状态和风险结论保存在 `cases/manifest.launch-release-20260904-r3.json` 与 `cases/published/launch-20260904-r3/`；`npm run cases:validate:dialogue` 和 `npm run cases:validate` 均按 v2 活动清单验证，不以 AI 审核结论充当技术阻塞门。
...
- 当前 C7 病例证据位于 `evaluation/phase8/c7-ai-release-20260904-r3/`：30 例、60 次独立 reviewer 调用、360 项检查均完成，实际模型为 `gpt-5.6-sol`；25 例 AI decision 为 `rejected` 并作为非阻塞风险原样保留。
- 当前 C7 对话证据位于 `evaluation/phase8/c7-dialogue-live-20260904-r2/`：30 例、360 个已提交轮，Patient 回复率 100%、人格一致率 99.72%、上下文跟进率 95%、自然语言检查动作正确率 78.33%；独立审计为 `rejected`，且记录 79 个严重事实错误，不能宣称质量通过。
...
- E3 六人格专项 benchmark 已实现 72 条规则断言、固定 C01 医学锚点的六人格真实 runner、逐问题不变量检查、虚构病例安全预检、两个隔离 AI 审阅角色和完整 hash 绑定。当前 `evaluation/phase8/e3-persona-live-20260904-r2/` 保存 63 个已提交轮和六份 journey；只有 `talkative_digressive` 完成 14 轮，其余五人格未达到最少轮数，因此审核未运行、decision 为 `not_run`。报告 SHA-256 为 `d6bad672e6c2535f094ed31d031fd5d7d6c3d388607aa379d5febf535a380d25`。证据复验、工程链和 441/441 测试已通过，但不得把该记录解释为六人格质量通过。
- E5 发布证据工程及 M0/B1–B5 内容生产已完成。当前最终候选为 `evaluation/phase8/e5-full-release-20260904-r2/`：24 项本地门禁、30 例、120 条轨迹、300 条黄金向量、600 条病例样本、40 条新增域安全样本、60 次病例 AI 审核、72 条人格规则、63 个 E3 锚点轮、360 个 C7 发布对话轮和 30 个身份均写入同一不可变证据集；E4 的 live/storage/双 AI 审核均通过，客户端扫描零命中。精确文件数、扫描数及 SHA-256 以该目录的 acceptance、runtime manifest、index 与 `SHA256SUMS.json` 为准。quality decision 为 `reported_with_failures`：25 例病例审核被拒绝，C7 独立对话审计被拒绝，自然语言检查动作正确率 78.33%，严重事实错误非零，E3 仅 63/72，来源许可 AI 评估未运行。上述均为非阻塞质量风险，不是工程或制品缺失。
- 当前批准范围只有第三方 OpenAI-compatible Provider + `gpt-5.6-sol`；Claude 真实验证延期且非阻塞，不存在自动 failover。
- 当前对话实现已完成 AI 主导/完整历史/诊断意图纠偏并通过全部测试与覆盖率门；v0.4 真实模型本地 smoke 已验证非医疗角色补全与诊断意图分支，v0.5.0 的当前质量依据改为 E3 r2 与 30 例 C7 dialogue r2。旧 C6、旧 C7 runtime/dialogue、旧 runtime manifest 和旧 Software RC 只作历史证据；E5 r2 是当前源码与文档对应的候选，quality decision 为 `reported_with_failures`。
