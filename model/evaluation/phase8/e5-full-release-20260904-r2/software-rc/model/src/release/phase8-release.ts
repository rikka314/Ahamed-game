...
export interface RuntimeReleaseQualityFindingV1 {
  code: string;
  scope: string;
  message: string;
}

export interface RuntimeReleaseQualityReportV1 {
  reviewPolicy: "non_blocking";
  findings: RuntimeReleaseQualityFindingV1[];
}
...
export interface RuntimeReleaseManifestV1 {
  ...
  qualityReport?: RuntimeReleaseQualityReportV1;
...
export function buildRuntimeReleaseManifest(input: {
  ...
  if (
    ...
    (input.provider.actualModelId === undefined
      ? input.provider.observationStatus !== "not_observed"
...
export function verifyRuntimeReleaseManifest(
  ...
  if (
    ...
    (manifest.qualityReport.reviewPolicy !== "non_blocking" ||
      !Array.isArray(manifest.qualityReport.findings) ||
      manifest.qualityReport.findings.some(
        ({ code, scope, message }) =>
          !nonEmptyString(code) || !nonEmptyString(scope) || !nonEmptyString(message),
      ))
  ) {
    throw new Error("runtime release quality report is invalid");
  }
