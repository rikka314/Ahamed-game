...
export const OPENAI_STRUCTURED_OUTPUT_SCHEMA_VERSION = "phase8-v3";
    ...
    properties: {
      ...
      diagnosisIntent: {
        ...
        properties: {
          decision: {
            type: "string",
            enum: ["continue_dialogue", "submit_diagnosis"],
          },
          primaryDiagnosis: {
            anyOf: [{ type: "string" }, { type: "null" }],
          },
...
function parsePatientDiagnosisIntent(
  value: unknown,
): PatientDiagnosisIntent | undefined {
  // Legacy pre-v0.4 fixtures and pending buffers safely mean "keep chatting".
  ...
  if (
    ...
    !isUniqueStrings(value["differentialDiagnoses"]) ||
    value["differentialDiagnoses"].length > 5 ||
    !isUniqueStrings(value["candidateDiagnoses"]) ||
    value["candidateDiagnoses"].length > 6
  ) {
    throw new ModelProviderOutputError(
      "Patient diagnosis intent failed validation.",
    );
  }
  if (
    value["decision"] === "continue_dialogue" &&
    value["primaryDiagnosis"] === null &&
    value["differentialDiagnoses"].length === 0 &&
    value["candidateDiagnoses"].every(
    ...
    return {
      ...
      differentialDiagnoses: [],
      candidateDiagnoses: value["candidateDiagnoses"],
    };
  }
  if (
    value["decision"] === "submit_diagnosis" &&
    typeof value["primaryDiagnosis"] === "string" &&
    value["primaryDiagnosis"].trim().length > 0 &&
    value["primaryDiagnosis"].length <= 200 &&
    value["candidateDiagnoses"].length === 0 &&
    value["differentialDiagnoses"].every(
      (item) => item.trim().length > 0 && item.length <= 200,
    )
  ) {
    return {
...
function parsePatient(
  ...
  const parsedDiagnosisIntent = isRecord(value)
    ? parsePatientDiagnosisIntent(value["diagnosisIntent"])
    : undefined;
  if (
    !isRecord(value) ||
    !hasExactKeys(value, [
  ...
  return {
    ...
    ...(parsedDiagnosisIntent === undefined
      ? {}
      : { diagnosisIntent: parsedDiagnosisIntent }),
...
export class OpenAIModelProvider implements ModelProvider, ReviewProvider {
  ...
  constructor(private readonly options: OpenAIModelProviderOptions) {
    ...
    this.identity = Object.freeze({
      ...
      promptVersion: buildPromptSetVersion(options.promptVersion, this.prompts),
    });
