...
test("E5 static scan fails closed when either required Next build root is missing", () => {
  const root = mkdtempSync(join(tmpdir(), "ahamed-e5-client-missing-root-"));
  try {
    const publicRoot = join(root, "public");
    mkdirSync(publicRoot, { recursive: true });
    writeFileSync(join(publicRoot, "clean.json"), '{"status":"ready"}\n');

    const result = scanE5StaticClientArtifacts(root);

    assert.equal(result.status, "failed");
    assert.equal(result.sensitiveMatches, 2);
    assert.ok(result.matches.some(({ token }) => token === "next_static_root_missing"));
    assert.ok(result.matches.some(({ token }) => token === "next_prerender_root_missing"));
  } finally {
    rmSync(root, { recursive: true, force: true });
  }
});
