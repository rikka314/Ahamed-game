...
`@ahamed/doctor-game-share@1.0.0-rc.1` 是无第三方运行时依赖的独立 TypeScript package。它提供公共类型、语言中立 JSON Schema、合成 fixture、版本清单和契约测试；`share` 不依赖 `game` 或 `model`。对话架构 C5 在保留该标签的前提下为 `TurnCompletedV1` 增加必填 `effects`，这是已记录的不兼容原地升级；旧 RC consumer/producer 必须更新后才能互操作。
