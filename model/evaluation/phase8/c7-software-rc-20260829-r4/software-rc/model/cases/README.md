...
> C0 状态：旧 `manifest.v1-rc1.json` 及其 published/AI-validation 绑定只读保留并已 `superseded`。C1 新对话候选记录在 `manifest.dialogue-candidate.v1-rc1.json`，当前仅结构和 canonical hash 就绪，尚未经过 C7 双 AI 复核，不得发布。绑定状态见 `manifest.v1-rc1.status.json`。
...
历史 C01–C05 published 病例、双 AI sidecar 与 `manifest.v1-rc1.json` 已按 C0 只读保留并标记为 `superseded`。当前 `1.0.0-draft.1` 对话候选的新 hash 只写入 `manifest.dialogue-candidate.v1-rc1.json`，5/5 structurally ready、0 published；必须在 C7 重新完成临床安全与诊断质量双 AI 验证后，才可生成新 published 病例和新发布 manifest。
