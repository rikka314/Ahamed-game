...
> 当前基线（2026-08-28）：对话架构重构 C0–C5 candidate。旧 Phase 8 Software RC、manifest 与测试证据已统一标记为 `superseded`；`1` / `v1-rc1` 标签继续保留，但属于不兼容原地升级。当前代码不是新的 Software RC，必须完成 C6–C7 和全量重新验收后才能恢复发布资格。机器可读状态见 `evaluation/phase8/supersession-status.v1.json`。
...
- 当前对话重构：**C0–C5 已完成，C6–C7 待执行**。只有完成 CLI 长旅程、五病例三人格真实 Provider 基准、双 AI 全量重新验收并生成新 Software RC 后，才可继续目标服务器 Phase 8-B 验证。
