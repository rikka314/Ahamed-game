...
function isPublicEvidenceFile(path: string): boolean {
  ...
  return /(?:(?:candidate-benchmark-report|candidate-quality-safety-report|c6-cli-acceptance|c7-runtime-acceptance|c7-dialogue-architecture-report|dialogue-sample-ai-validation|provider-model-approval|e3-persona-(?:ai-cross-review|benchmark-report|rule-corpus)|e5-full-acceptance-report|runtime-release-manifest|software-rc-index|share-version-decision)[^/]*\.json|raw\/.*\.json)$/u.test(
    path,
  );
}
