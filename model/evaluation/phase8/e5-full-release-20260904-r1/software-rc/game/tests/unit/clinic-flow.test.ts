...
describe("clinic flow", () => {
  ...
  it("keeps the seat exclusive and advances to the second patient only after departure", () => {
    ...
    const staleDeparture = transitionClinicFlow(state, {
      type: "patient.departure.start",
      commandId: "departure-start-stale",
      npcId: GRAYBOX_PATIENTS[0].npcId,
      sessionId: GRAYBOX_PATIENTS[1].sessionId,
    });
    expect(staleDeparture).toMatchObject({
      status: "rejected",
      reason: "departure_requires_seated_current_patient",
    });

    state = apply(state, {
      type: "patient.departure.start",
      commandId: `dismiss.${GRAYBOX_PATIENTS[0].sessionId}`,
      npcId: GRAYBOX_PATIENTS[0].npcId,
      sessionId: GRAYBOX_PATIENTS[0].sessionId,
    });
    state = apply(state, {
      type: "patient.departure.complete",
      commandId: `departure-complete.${GRAYBOX_PATIENTS[0].sessionId}`,
      npcId: GRAYBOX_PATIENTS[0].npcId,
      sessionId: GRAYBOX_PATIENTS[0].sessionId,
    });
    ...
    expect(state.completedSessionIds).toEqual([GRAYBOX_PATIENTS[0].sessionId]);
  ...
  it("rejects duplicate patient roles and session bindings before opening a shift", () => {
    let state = createClinicFlowState();
    state = apply(state, { type: "intro.complete", commandId: "intro-01" });
    state = apply(state, { type: "computer.open", commandId: "computer-open-01" });

    const duplicateRole = transitionClinicFlow(state, {
      type: "shift.start",
      commandId: "start-shift-role",
      shiftId: GRAYBOX_SHIFT_ID,
      patients: [
        GRAYBOX_PATIENTS[0],
        {
          ...GRAYBOX_PATIENTS[1],
          patientRoleId: GRAYBOX_PATIENTS[0].patientRoleId,
        },
      ],
    });
    expect(duplicateRole).toMatchObject({
      status: "rejected",
      reason: "patient_bindings_must_be_unique",
    });

    const duplicateSession = transitionClinicFlow(state, {
      type: "shift.start",
      commandId: "start-shift-session",
      shiftId: GRAYBOX_SHIFT_ID,
      patients: [
        GRAYBOX_PATIENTS[0],
        { ...GRAYBOX_PATIENTS[1], sessionId: GRAYBOX_PATIENTS[0].sessionId },
      ],
    });
    expect(duplicateSession).toMatchObject({
      status: "rejected",
      reason: "patient_bindings_must_be_unique",
    });
  });
