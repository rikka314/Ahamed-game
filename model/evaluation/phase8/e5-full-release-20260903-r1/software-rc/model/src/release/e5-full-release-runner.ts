...
const TEXT_EXTENSIONS = new Set(["", ".cjs", ".css", ".html", ".js", ".json", ".map", ".md", ".mjs", ".ts", ".txt", ".yaml", ".yml"]);
...
export function parseE5FullReleaseArguments(argv: readonly string[]): Arguments {
  ...
  return {
    outputDirectory,
    e3EvidenceDirectory: values.get("--e3-evidence") ?? DEFAULT_E3_EVIDENCE,
  };
}
...
export function scanE5StaticClientArtifacts(gamePackageRoot: string): {
  status: "passed" | "failed" | "not_run";
  scannedFiles: number;
  sensitiveMatches: number;
  matches: Array<{ path: string; token: string }>;
} {
  const roots = [resolve(gamePackageRoot, ".next"), resolve(gamePackageRoot, "public")];
  const candidates = roots.flatMap(walkFiles).filter((path) => TEXT_EXTENSIONS.has(extname(path).toLowerCase()));
  const matches: Array<{ path: string; token: string }> = [];
  for (const path of candidates) {
    ...
    for (const token of CLIENT_FORBIDDEN_TOKENS) {
      if (content.toLocaleLowerCase("en-US").includes(token.toLocaleLowerCase("en-US"))) {
        matches.push({ path: portableRelative(gamePackageRoot, path), token });
      }
    }
    for (const [index, pattern] of CLIENT_SECRET_PATTERNS.entries()) {
      pattern.lastIndex = 0;
      if (pattern.test(content)) matches.push({ path: portableRelative(gamePackageRoot, path), token: `secret_pattern_${index + 1}` });
    }
  }
  return {
...
function main(): void {
  ...
  try {
    ...
    for (const [name, script, cwd, timeoutMs] of commandPlan) {
      commands.push(runE5NpmCommand({ name, script, cwd, timeoutMs }));
    }
    ...
    const evidenceBindings: E5EvidenceBinding[] = [
      ...
      collectEvidenceBinding(modelRoot, "e3-persona-report", e3ReportPath, "current"),
    ...
    const stagedPaths = [...manifestValue.artifacts.map(({ path }) => path), MANIFEST_FILENAME];
