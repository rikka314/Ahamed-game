...
import { C7_ACCEPTANCE_TEST_PATHS } from "./c7-runtime-release.js";
...
function main(): void {
  try {
    ...
    const compiledTests = C7_ACCEPTANCE_TEST_PATHS.map((file) =>
      resolve(modelRoot, "dist", file.replace(/\.ts$/u, ".js"))
    );
    ...
    if (result.error !== undefined || result.status === null) {
      throw new Error(
        `C7 acceptance command could not execute: ${result.error?.message ?? result.signal ?? "unknown process failure"}`,
      );
    }
    ...
    const evidence = {
      ...
      findings: status === "passed"
        ? []
        : [{
            code: "C7_ACCEPTANCE_TESTS_FAILED",
            message: `test command exited with code ${result.status}`,
          }],
      ...
      testFiles: C7_ACCEPTANCE_TEST_PATHS.map((path) => ({
        path,
        sha256: sha256(readFileSync(resolve(modelRoot, path))),
      })),
      verification: {
    ...
    process.stdout.write(`${JSON.stringify({
      ...
      testFiles: C7_ACCEPTANCE_TEST_PATHS.length,
