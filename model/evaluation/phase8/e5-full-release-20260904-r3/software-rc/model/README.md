...
- E3 六人格专项 benchmark 已实现 72 条规则断言、固定 C01 医学锚点的六人格真实 runner、逐问题不变量检查、虚构病例安全预检、两个隔离 AI 审阅角色和完整 hash 绑定。当前 `evaluation/phase8/e3-persona-live-20260904-r2/` 保存 63 个已提交轮和六份 journey；只有 `talkative_digressive` 完成 14 轮，其余五人格未达到最少轮数，因此审核未运行、decision 为 `not_run`。报告 SHA-256 为 `d6bad672e6c2535f094ed31d031fd5d7d6c3d388607aa379d5febf535a380d25`。证据复验、工程链和当前 447/447 测试已通过，但不得把该记录解释为六人格质量通过。
- E5 发布证据工程及 M0/B1–B5 内容生产已完成。`evaluation/phase8/e5-full-release-20260904-r2/` 是病例修订前的历史候选；其中 25 例病例拒绝已由 r9 的 30/30 病例批准证据取代。完整 E5 尚未基于 r9 重新生成，因此不能继续把 r2 表述为当前源码候选。C7 独立对话审计拒绝、自然语言检查动作正确率 78.33%、严重事实错误非零、E3 仅 63/72 和来源许可 AI 评估未运行仍是独立的非病例风险。
- 当前批准范围只有第三方 OpenAI-compatible Provider + `gpt-5.6-sol`；Claude 真实验证延期且非阻塞，不存在自动 failover。
- 当前对话实现已完成 AI 主导/完整历史/诊断意图纠偏并通过 447/447 测试与全部覆盖率门；v0.4 真实模型本地 smoke 已验证非医疗角色补全与诊断意图分支，v0.5.0 的对话质量依据仍是 E3 r2 与 30 例 C7 dialogue r2。病例质量依据已更新为 C7 AI release r9；旧 C6、旧 C7 runtime/dialogue、旧 runtime manifest、旧 Software RC 和 E5 r2 只作历史证据，新的完整 E5 尚待基于 r9 与更新后的非病例证据重建。
