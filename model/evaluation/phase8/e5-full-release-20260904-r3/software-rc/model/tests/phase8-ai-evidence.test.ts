...
import { assertC7SafetyCorpusEvidence } from "../src/release/c7-runtime-release.js";
...
class ScriptedAuditTransport implements OpenAIResponsesTransport {
  ...
  constructor(
    ...
    private readonly shouldReturnMalformed?: (
      request: OpenAITransportRequest,
      sequence: number,
    ) => boolean,
  ) {}
  ...
  async create(request: OpenAITransportRequest): Promise<OpenAITransportResponse> {
    ...
    this.sequence += 1;
    if (this.shouldFail?.(request, this.sequence) === true) {
      throw new Error(`scripted provider failure ${this.sequence}`);
    }
    ...
    return {
      status: "completed",
      outputText: this.shouldReturnMalformed?.(request, this.sequence) === true
        ? "{"
...
test("Phase 8 case validation recovers from two transient transport failures", async () => {
  assert.ok(publishedCase);
  const transport = new ScriptedAuditTransport(
    undefined,
    (_request, sequence) => sequence <= 2,
  );
  const validation = await generatePhase8CaseValidation({
    casePackage: publishedCase,
    transport,
...
test("Phase 8 case validation does not retry non-retryable Provider failures", async () => {
  assert.ok(publishedCase);
  const requests: OpenAITransportRequest[] = [];
  const transport: OpenAIResponsesTransport = {
    providerName: "openai-compatible.non-retryable-test",
    protocol: "openai-responses",
    endpointSha256: "f".repeat(64),
    async create(request): Promise<OpenAITransportResponse> {
  ...
  assert.equal(validation.decision, "not_run");
  assert.equal(requests.length, 2);
  assert.equal(
    requests.filter(({ schema }) => schema.name.includes("clinical_safety")).length,
    1,
  );
  assert.equal(
    requests.filter(({ schema }) => schema.name.includes("diagnostic_quality")).length,
    1,
  );
});
...
test("Phase 8 safety audit preserves a completed counterpart when the other role fails", async () => {
  const samples: Phase7SafetyCorpusSample[] = [
    {
      sampleId: "sample-partial-001",
      text: "游戏里的虚构患者胸痛。",
      label: "GAME_IN_SCOPE",
      person: "fictional",
      population: "fictional",
  ...
  const artifact = await generatePhase8SafetyCorpusValidation({
    samples,
    transport: new ScriptedAuditTransport(undefined, (request) =>
      request.clientRequestId === "phase8_safety_safety_label_auditor_1"),
    modelId: "gpt-configured",
    policyVersion: "medical-safety-policy-v1",
    templateRegistry: { ALLOW_GAME: "safety.allow-game.zh-CN.v1" },
    batchSize: 1,
  });

  assert.equal(artifact.decision, "not_run");
  const failed = artifact.validations[0]!;
  const completed = artifact.validations[1]!;
  assert.equal(failed.runStatus, "failed_to_run");
  assert.equal(failed.decision, "not_run");
  assert.equal(failed.modelId, "gpt-test-snapshot");
  assert.equal(failed.assessedSamples, 1);
  assert.match(failed.failureCode ?? "", /scripted provider failure/u);
  assert.equal(completed.runStatus, "completed");
  assert.equal(completed.modelId, "gpt-test-snapshot");
  assert.equal(completed.assessedSamples, samples.length);
});

test("Phase 8 safety audit records a truthful not-run result when both roles fail", async () => {
  const samples: Phase7SafetyCorpusSample[] = [{
    sampleId: "sample-outage-001",
    text: "游戏里的虚构患者胸痛。",
    label: "GAME_IN_SCOPE",
    person: "fictional",
    population: "fictional",
  ...
  const artifact = await generatePhase8SafetyCorpusValidation({
    samples,
    transport: new ScriptedAuditTransport(undefined, () => true),
    modelId: "gpt-configured",
    policyVersion: "medical-safety-policy-v1",
    templateRegistry: { ALLOW_GAME: "safety.allow-game.zh-CN.v1" },
    batchSize: 1,
  ...
  assert.equal(artifact.decision, "not_run");
  assert.equal(artifact.validations.length, 2);
  assert.ok(artifact.validations.every(({ runStatus }) => runStatus === "failed_to_run"));
  assert.ok(artifact.validations.every(({ decision }) => decision === "not_run"));
  assert.ok(artifact.validations.every(({ modelId }) => modelId === "gpt-configured"));
  assert.ok(artifact.validations.every(({ assessedSamples }) => assessedSamples === 0));
});

test("Phase 8 safety audit reports malformed Provider output without aborting", async () => {
  const samples: Phase7SafetyCorpusSample[] = [{
    sampleId: "sample-malformed-001",
    text: "游戏里的虚构患者胸痛。",
    label: "GAME_IN_SCOPE",
    person: "fictional",
    population: "fictional",
  ...
  const artifact = await generatePhase8SafetyCorpusValidation({
    samples,
    transport: new ScriptedAuditTransport(undefined, undefined, () => true),
    modelId: "gpt-configured",
    policyVersion: "medical-safety-policy-v1",
    templateRegistry: { ALLOW_GAME: "safety.allow-game.zh-CN.v1" },
    batchSize: 1,
  ...
  assert.equal(artifact.decision, "not_run");
  assert.ok(artifact.validations.every(({ failureCode }) =>
    failureCode === "INVALID_PROVIDER_REVIEW_RESPONSE"));
  assert.ok(artifact.validations.every(({ subcallCount }) => subcallCount === 1));
  assert.ok(artifact.validations.every(({ modelId }) => modelId === "gpt-test-snapshot"));
  assert.doesNotThrow(() => assertC7SafetyCorpusEvidence(artifact, {
    configuredModelId: "gpt-configured",
    actualModelId: "gpt-test-snapshot",
    datasetVersion: "phase7-medical-safety-corpus-v1",
    policyVersion: "medical-safety-policy-v1",
    corpusHash: artifact.corpusHash,
    holdoutHash: artifact.holdoutHash,
    templateRegistryHash: artifact.templateRegistryHash,
    totalSamples: 1,
    holdoutSamples: 1,
    expectedDecisionsBySampleId: { "sample-malformed-001": "ALLOW_GAME" },
  }));
});
...
test("Phase 8 patient reply audit remains non-blocking and preserves its completed counterpart", async () => {
  const samples = [
    {
      sampleId: "case-001.run-1.turn-1",
      caseId: "case-001",
      ...
      reply: "大约三天前。",
      disclosedFactIds: ["fact.onset"],
      authorizedFacts: [{ factId: "fact.onset", status: "present" as const, value: "三天前" }],
      forbiddenDiagnosisTerms: ["普通感冒"],
    },
    {
      ...
      question: "还有哪里不舒服？",
      reply: "有点咳嗽。",
      disclosedFactIds: ["fact.cough"],
      authorizedFacts: [{ factId: "fact.cough", status: "present" as const, value: "咳嗽" }],
      forbiddenDiagnosisTerms: ["普通感冒"],
    },
  ];
  const report = await generatePhase8PatientSampleValidation({
    samples,
    transport: new ScriptedAuditTransport(undefined, (request) =>
      request.clientRequestId === "phase8_patient_sample_fact_safety_auditor_1"),
    modelId: "gpt-configured",
    candidateRunSetSha256: "c".repeat(64),
    minimumSamples: 2,
    batchSize: 1,
  });

  assert.equal(report.decision, "not_run");
  assert.equal(report.validations[0]!.runStatus, "failed_to_run");
  assert.equal(report.validations[0]!.modelId, "gpt-test-snapshot");
  assert.equal(report.validations[0]!.assessedSamples, 1);
  assert.equal(report.validations[1]!.runStatus, "completed");
  assert.equal(report.validations[1]!.modelId, "gpt-test-snapshot");
  assert.equal(report.validations[1]!.assessedSamples, 2);
});

test("Phase 8 patient reply audit records insufficient live samples as not_run", async () => {
  const transport = new ScriptedAuditTransport();
  let runSequence = 0;
  const report = await generatePhase8PatientSampleValidation({
    samples: [],
    transport,
    modelId: "gpt-configured",
    candidateRunSetSha256: "b".repeat(64),
    minimumSamples: 360,
  ...
  assert.equal(report.naturalAndRoleConsistentRate, 0);
  assert.equal(new Set(report.validations.map(({ validationRunId }) => validationRunId)).size, 2);
  assert.ok(report.validations.every((validation) =>
    validation.runStatus === "failed_to_run" &&
    validation.decision === "not_run" &&
    validation.modelId === "gpt-configured" &&
    validation.failureCode === "INSUFFICIENT_PATIENT_SAMPLES"));
});

test("Phase 8 patient reply audit reports malformed Provider output without aborting", async () => {
  const samples = [{
    sampleId: "case-001.run-1.turn-1",
    caseId: "case-001",
    caseVersion: "1.0.0",
  ...
  assert.equal(report.decision, "not_run");
  assert.ok(report.validations.every(({ failureCode }) =>
    failureCode === "INVALID_PROVIDER_REVIEW_RESPONSE"));
  assert.ok(report.validations.every(({ subcallCount }) => subcallCount === 1));
  assert.ok(report.validations.every(({ modelId }) => modelId === "gpt-test-snapshot"));
});
...
test("Phase 8 permits a natural fact-free social reply without a fixed phrase allowlist", async () => {
  const transport = new ScriptedAuditTransport(
    (request) => !request.instructions.includes("事实与安全审计角色"),
  );
  const report = await generatePhase8PatientSampleValidation({
    samples: [
      {
        ...
        caseVersion: "1.0.0",
        question: "散步时一般去哪里？",
        reply: "我一般沿着校园湖边慢慢走，晚上那里比较安静。",
        disclosedFactIds: [],
        authorizedFacts: [],
        authorizedPersonaFacts: [
          { personaFactId: "persona.interest.1", value: "喜欢散步" },
        ],
