...
test("persona registry returns defensive copies of all three legacy templates", () => {
  const templates = listPatientPersonaTemplates(PATIENT_PERSONA_TEMPLATE_VERSION_V1);
  assert.deepEqual(
  ...
  assert.notEqual(
    listPatientPersonaTemplates(PATIENT_PERSONA_TEMPLATE_VERSION_V1)[0]!
      .behaviorInstructions[0],
    "mutated test value",
  );
});
...
test("safe patient view excludes hidden truth and uncompleted test reports", () => {
  ...
  assert.equal(view.patientProfile.displayName, "林同学");
  assert.equal(view.patientProfile.ageBand, "18–25岁");
  assert.equal(view.patientProfile.genderDisplay, "女");
  assert.deepEqual(
    view.patientProfile.personaFacts.find(
      ({ personaFactId }) => personaFactId === "persona.scene_label",
    ),
    {
  ...
  assert.deepEqual(
    view.patientProfile.personaFacts
      .filter(({ personaFactId }) =>
        ["persona.age_band", "persona.gender"].includes(personaFactId)
      ),
    [
      { personaFactId: "persona.age_band", value: "18–25岁" },
      { personaFactId: "persona.gender", value: "女" },
    ],
  );
  assert.ok(
