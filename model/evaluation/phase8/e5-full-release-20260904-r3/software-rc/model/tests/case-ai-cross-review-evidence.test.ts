...
test("v3 case cross-review records failed_to_run and still invokes the isolated counterpart", async () => {
  ...
  assert.equal(delegate.requests.filter(({ schema }) => schema.name.includes("clinical_safety")).length, 3);
  assert.equal(delegate.requests.filter(({ schema }) => schema.name.includes("diagnostic_quality")).length, 1);
});

test("case review evidence accepts a third-attempt recovery and rejects calls after success", async () => {
  class ThirdAttemptTransport extends StubTransport {
    private clinicalAttempts = 0;

    override async create(
      request: OpenAITransportRequest,
    ): Promise<OpenAITransportResponse> {
      if (request.schema.name.includes("clinical_safety")) {
        this.clinicalAttempts += 1;
        if (this.clinicalAttempts <= 2) {
  ...
  const delegate = new ThirdAttemptTransport();
  const transport = new ObservedCaseReviewTransport(delegate);
  const review = await generatePhase8CaseCrossReviewV3({
    casePackage: draftCase(),
    transport,
    modelId: "configured-model",
    now: () => new Date("2026-09-03T00:00:00.000Z"),
    runId: (role) => `run.${role}.third-attempt`,
  });
  const root = mkdtempSync(join(tmpdir(), "ahamed-case-review-third-attempt-"));
  try {
    ...
    assert.deepEqual(
      evidence.attempts
        .filter(({ role }) => role === "clinical_safety")
        .map(({ attempt, status }) => ({ attempt, status })),
      [
        { attempt: 1, status: "failed_to_run" },
        { attempt: 2, status: "failed_to_run" },
    ...
    const afterSuccess = structuredClone(evidence);
    const successfulAttempt = afterSuccess.attempts.find(
      ({ role, status }) => role === "clinical_safety" && status === "completed",
    )!;
    const failedAttempt = afterSuccess.attempts.find(
      ({ role, attempt }) => role === "clinical_safety" && attempt === 2,
    )!;
    afterSuccess.attempts = afterSuccess.attempts.map((attempt) => {
      if (attempt.role !== "clinical_safety") return attempt;
      if (attempt.attempt === 2) return { ...successfulAttempt, attempt: 2 };
      if (attempt.attempt === 3) return { ...failedAttempt, attempt: 3 };
      return attempt;
    });
    afterSuccess.attemptsSha256 = sha256Canonical(afterSuccess.attempts);
    assert.throws(
      () => verifyCaseReviewEvidence(afterSuccess, root),
      /retry or invocation sequence is invalid/u,
    );
  } finally {
