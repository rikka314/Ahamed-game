...
import { isC7DialogueProviderCompatible } from "../src/evaluation/c7-dialogue-evidence-verify.js";
import type {
...
const RELEASE_POLICY = toC7DialogueReleasePolicy(
  loadCaseManifestV2("cases/manifest.phase6-compat.v2-rc9.json").releasePolicy,
);
  ...
  const dialogueJourneys: C7RuntimeReleaseEvidence["dialogueJourneys"] =
    publishedCasePackages.map((casePackage) => {
      ...
      turns: Array.from({ length: 12 }, (_, turnIndex) => ({
        ...
        reply: turnIndex === 11
          ? `${casePackage.publicCaseId}: ${benchmarkTest.report}`
        ...
        interactionKind: turnIndex === 9
          ? "test_query"
          : turnIndex === 8 || turnIndex === 10
          ? "test_order"
          : turnIndex === 11
          ? "test_query"
          : "medical_chat",
        personaFactIdsUsed: [],
        completedTestIdsUsed: turnIndex === 11 ? [benchmarkTest.testId] : [],
        effects: turnIndex === 8
          ? [{
              type: "test_completed",
          ...
          ? [{
              type: "test_completed",
              result: { testId: benchmarkTest.testId, report: benchmarkTest.report },
            }]
          : [],
        ...
        sessionSnapshotBeforeTurn: turnIndex === 10
          ? {
              pendingTestSuggestionId: benchmarkTest.testId,
              completedTests: [],
            }
          : turnIndex === 11
          ? {
...
test("C7 runtime release binds a malformed-response audit failure to the observed model", () => {
  const candidate = approvedEvidence();
  const failedAudit = candidate.dialogueAudit.validations[0]!;
  failedAudit.modelId = candidate.aiIndex.provider.actualModelId!;
  failedAudit.runStatus = "failed_to_run";
  failedAudit.decision = "not_run";
  failedAudit.assessedSamples = 0;
  failedAudit.seriousErrors = 0;
  failedAudit.unauthorizedFactLeaks = 0;
  failedAudit.diagnosisLeaks = 0;
  failedAudit.unknownAsAbsentErrors = 0;
  failedAudit.naturalAndRoleConsistent = 0;
  failedAudit.subcallCount = 1;
  failedAudit.providerRequestIds = [];
  failedAudit.usage = { inputTokens: 0, outputTokens: 0, totalTokens: 0 };
  failedAudit.assessments = [];
  failedAudit.failureCode = "INVALID_PROVIDER_AUDIT_RESPONSE";
  candidate.dialogueAudit.decision = "not_run";

  const previousReport = candidate.dialogueReport;
  candidate.dialogueReport = {
    ...buildC7DialogueArchitectureReport({
      runs: previousReport.runs,
    ...
    runSetSha256: previousReport.runSetSha256,
    provider: previousReport.provider,
    bindings: previousReport.bindings,
  };
...
test("C7 dialogue reconstruction preserves a zero-turn Provider outage as reported evidence", () => {
  const baseline = approvedEvidence();
  const journey = structuredClone(baseline.dialogueJourneys[0]!);
  journey.runStatus = "failed_to_run";
  journey.failureCode = "MODEL_OUTPUT_REJECTED";
  journey.uncommittedAttempt = {
    patientProviderCalls: 2,
    controllerProviderCalls: 0,
  };
  journey.turns = [];
  ...
  assert.equal(result.runs[0]!.committedTurns, 0);
  assert.equal(result.runs[0]!.patientProviderCalls, 2);
  assert.equal(result.runs[0]!.failureCode, "MODEL_OUTPUT_REJECTED");
  assert.deepEqual(result.samples, []);
});

test("C7 late partial outages use the same benchmark accounting as live evidence", () => {
  const baseline = approvedEvidence();
  for (const [turnCount, expected] of [
    [10, { evaluated: 1, correct: 1, states: ["not_completed"] }],
    [11, { evaluated: 3, correct: 3, states: ["not_completed", "pending_confirmation"] }],
  ] as const) {
    const journey = structuredClone(baseline.dialogueJourneys[0]!);
    journey.runStatus = "failed_to_run";
    journey.failureCode = "MODEL_UNAVAILABLE";
    journey.turns = journey.turns.slice(0, turnCount);
    const result = reconstructC7DialogueEvidence({
      casePackages: [baseline.publishedCasePackages[0]!],
      caseBindings: [baseline.caseManifest.publishedCases[0]!],
      journeys: [journey],
    ...
    assert.equal(result.runs[0]!.testActionsEvaluated, expected.evaluated);
    assert.equal(result.runs[0]!.testActionsCorrect, expected.correct);
    assert.deepEqual(result.runs[0]!.observedTestStates, expected.states);
  }
});

test("C7 Provider binding permits a dialogue outage after successful upstream reviews", () => {
  const upstream = approvedEvidence().aiIndex.provider;
  const { actualModelId: _actualModelId, ...observed } = upstream;
  assert.equal(isC7DialogueProviderCompatible(observed, upstream), true);
  assert.equal(isC7DialogueProviderCompatible(
    { ...observed, configuredModelId: "drifted-model" },
    upstream,
  ), false);
});
...
test("C7 semantic evidence checks accept internally consistent rejected decisions", () => {
  ...
  const partialNotRunSafety = structuredClone(safety);
  partialNotRunSafety.decision = "not_run";
  partialNotRunSafety.validations[0] = {
    ...partialNotRunSafety.validations[0]!,
  ...
  partialNotRunSafety.validations[1]!.runStatus = "completed";
  assert.doesNotThrow(() => assertC7SafetyCorpusEvidence(partialNotRunSafety, {
    ...expected,
    configuredModelId: "model-configured",
  }));
  const failedModelTamper = structuredClone(partialNotRunSafety);
  failedModelTamper.validations[0]!.modelId = "model-approved";
  assert.throws(
    () => assertC7SafetyCorpusEvidence(failedModelTamper, {
      ...expected,
      configuredModelId: "model-configured",
    }),
    /not-run evidence binding drifted/u,
  );
  const completeOutageSafety = structuredClone(partialNotRunSafety);
  completeOutageSafety.validations[1] = {
    ...completeOutageSafety.validations[1]!,
    modelId: "model-configured",
    runStatus: "failed_to_run",
    decision: "not_run",
  ...
  assert.doesNotThrow(() => assertC7SafetyCorpusEvidence(completeOutageSafety, {
    ...expected,
