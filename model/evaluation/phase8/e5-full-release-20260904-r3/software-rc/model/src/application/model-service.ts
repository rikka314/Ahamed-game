...
export type DiagnosisEvaluationRequestStatus =
  | IdempotencyStatus
  | "not_found";
...
const DIAGNOSIS_DECLARATION_PREFIX_PATTERNS = [
  /^(?:我(?:的)?\s*)?(?:(?:最终|初步|临床)\s*)?诊断(?:考虑|判断)?(?:是|为|[:：])/u,
  /^(?:my\s+)?(?:(?:final|working|clinical)\s+)?diagnosis\s*(?:is|[:：])/iu,
] as const;

const DIAGNOSIS_DECLARATION_PATTERNS = [
  /^(?:我(?:的)?\s*)?(?:(?:最终|初步|临床)\s*)?诊断(?:考虑|判断)?(?:是|为|[:：])\s*(?:(?=[^,，;；。]{0,79}\p{Script=Han})[\p{L}\p{N}+\-·（）()/'’]{1,80}|[a-z0-9][a-z0-9+.'’()/-]{0,39})(?:\s*[,，;；]\s*(?:鉴别(?:诊断)?(?:考虑)?|需与|排除)\s*[:：]?\s*[\p{L}\p{N}+\-·、，,（）()/'’\s]{1,100})?[。.\s]*$/u,
  /^(?:my\s+)?(?:(?:final|working|clinical)\s+)?diagnosis\s*(?:is|[:：])\s*[a-z0-9][a-z0-9+.'’()/-]{0,39}(?:\s*[,;]\s*(?:(?:differential(?:\s+diagnos(?:is|es))?|differentials?)\s*(?:(?:is|are)\s+|[:：]\s*)?|differentiate\s+from\s+|rule\s+out\s+|v(?:s|ersus)\.?\s+)[a-z0-9][a-z0-9+.'’(),/\s-]{0,99})?[.\s]*$/iu,
] as const;

const DIAGNOSIS_EXTRACTION_CUES =
  /(?:标准答案|病例答案|答案键|隐藏诊断|评分表|得分点|诊断同义词|病例解析|评分规则|正确结论|真实结论|谜底|底牌|真相|系统提示|内部资料|(?:告诉|给|给出|说出|亮出|透露|公布|公开|揭示|披露|展示|输出|列出|说明|确认).{0,20}(?:答案|诊断|结果|结论|评分|谜底|底牌|真相|提示|资料)|(?:请|帮(?:我)?|替我|给我|直接|立即|马上|顺便|同时|另外|并且|然后|再).{0,20}(?:告诉|给出|说出|亮出|透露|公布|公开|揭示|披露|展示|输出|列出|说明|确认|答案|诊断|结果|结论|评分|谜底|底牌|真相|提示|资料)|\b(?:answer|solution|rubric|score|scoring|prompt|verdict|truth|result|answer\s*key|gold\s*answer|hidden\s*diagnosis|diagnosis\s*synonyms?|case\s*analysis)\b|(?:correct|right|official|actual|gold)\s+(?:answer|diagnosis|verdict|result)|(?:tell|give|show|reveal|publish|provide|supply|share|display|output|list|disclose|return|state|confirm).{0,40}(?:answer|diagnosis|solution|rubric|score|verdict|truth|result|scoring\s+rules?|case\s+analysis))/iu;

function normalizedDiagnosisDeclaration(text: string): string {
  return text
    .normalize("NFKC")
    .replace(/\p{Cf}+/gu, "")
    .trim();
...
function isDiagnosisDeclaration(text: string): boolean {
  const normalizedText = normalizedDiagnosisDeclaration(text);
  return DIAGNOSIS_DECLARATION_PATTERNS.some((pattern) =>
    pattern.test(normalizedText)
  );
}

function isDiagnosisDeclarationLike(text: string): boolean {
  const normalizedText = normalizedDiagnosisDeclaration(text);
  return DIAGNOSIS_DECLARATION_PREFIX_PATTERNS.some((pattern) =>
    pattern.test(normalizedText)
  );
}

function isDiagnosisExtractionAttempt(text: string): boolean {
  const normalizedText = normalizedDiagnosisDeclaration(text);
  return isDiagnosisDeclarationLike(normalizedText) &&
    (!isDiagnosisDeclaration(normalizedText) ||
      DIAGNOSIS_EXTRACTION_CUES.test(normalizedText));
}

function isPotentialDiagnosisSubmission(text: string): boolean {
  return isDiagnosisDeclaration(text) && !isDiagnosisExtractionAttempt(text);
}

const COMMITTED_DIAGNOSIS_TURN_TEXT = "玩家已提交结构化最终诊断。";

function diagnosisIntentFromDeclaration(text: string): PatientAgentOutput["diagnosisIntent"] {
  const normalizedText = normalizedDiagnosisDeclaration(text);
  const chinesePrefix = DIAGNOSIS_DECLARATION_PREFIX_PATTERNS[0].exec(
    normalizedText,
  );
  if (chinesePrefix !== null) {
    const declaration = normalizedText
      .slice(chinesePrefix[0].length)
      .replace(/[。.\s]+$/u, "")
      .trim();
    const split = declaration.split(
      /\s*[,，;；]\s*(?:鉴别(?:诊断)?(?:考虑)?|需与|排除)\s*[:：]?\s*/u,
      2,
    );
    const primaryDiagnosis = split[0]?.trim();
    if (primaryDiagnosis === undefined || primaryDiagnosis.length === 0) {
      return undefined;
    }
    const differentialDiagnoses = split[1] === undefined
      ? []
      : split[1]
          .split(/[、，,]/u)
          .map((item) => item.trim())
  ...
  const englishPrefix = DIAGNOSIS_DECLARATION_PREFIX_PATTERNS[1].exec(
    normalizedText,
  );
  if (englishPrefix === null) return undefined;
  const declaration = normalizedText
    .slice(englishPrefix[0].length)
    .replace(/[.\s]+$/u, "")
    .trim();
  const split = declaration.split(
    /\s*[,;]\s*(?:(?:differential(?:\s+diagnos(?:is|es))?|differentials?)\s*(?:(?:is|are)\s+|[:：]\s*)?|differentiate\s+from\s+|rule\s+out\s+|v(?:s|ersus)\.?\s+)/iu,
    2,
  );
  const primaryDiagnosis = split[0]?.trim();
  if (primaryDiagnosis === undefined || primaryDiagnosis.length === 0) {
...
function trustedDiagnosisTurnOutput(text: string): PatientAgentOutput | undefined {
  if (!isPotentialDiagnosisSubmission(text)) return undefined;
  const diagnosisIntent = diagnosisIntentFromDeclaration(text);
  if (diagnosisIntent === undefined) return undefined;
  return {
    reply: "收到，医生。诊断已提交。",
...
function diagnosisSafetyText(text: string): string {
  return text
    .replace(/^\s*我的(?=(?:最终|初步|临床)?\s*诊断)/u, "")
    .replace(/^\s*my\s+(?=(?:(?:final|working|clinical)\s+)?diagnosis)/iu, "");
}
...
function localSafetyInterruption(
  ...
  // A declaration-shaped diagnosis is extracted locally into a data-only
  // structure before any provider call. Medical safety still evaluates the
  // complete turn as a fictional-case message so trailing real-person
  // symptoms cannot inherit the diagnosis-submission exemption.
  const isTurnDiagnosis =
    surface === "turn" && isPotentialDiagnosisSubmission(text);
  const isTurnDiagnosisLike =
    surface === "turn" && isDiagnosisDeclarationLike(text);
  const safetySurface = isTurnDiagnosis ? "diagnosis_submission" : surface;
  let medicalResult: MedicalSafetyResultV1;
  try {
    const candidate: unknown = policy.evaluate({
      text:
        isTurnDiagnosisLike
          ? diagnosisSafetyText(text)
          : text,
      context: safetySurface === "diagnosis_submission" && !isTurnDiagnosis
  ...
  if (
    isDiagnosisExtractionAttempt(text) ||
    isPromptInjection(text, safetySurface)
  ) {
    return promptInjectionInterruption();
  }
  return undefined;
...
function readTurnBufferPayload(
  ...
  if (
    ...
    (committedText !== undefined && typeof committedText !== "string") ||
...
function validateAndRebuildTurnBufferPayload(
  ...
  if (payload.patientAgentOutput !== undefined) {
    ...
    return {
      ...
      ...(payload.committedText === undefined
        ? {}
...
export class ModelService {
  ...
  getDiagnosisEvaluationRequestStatus(
    ...
    assertPublicId(sessionId, "sessionId");
    assertPublicId(clientRequestId, "clientRequestId");
    if (idempotencyScopeId !== undefined) {
      assertPublicId(idempotencyScopeId, "idempotencyScopeId");
    }
    return this.transact((transaction, stagedEvents) => {
      ...
      this.assertSessionScope(session, idempotencyScopeId);
      return transaction.idempotency.get(
        sessionId,
        "submit_diagnosis",
        clientRequestId,
      )?.status ?? "not_found";
  ...
  private async executeTurnOperation(
    ...
    try {
      ...
      const trustedDiagnosisOutput = trustedDiagnosisTurnOutput(text);
      let validatedPatientReply: PatientAgentOutput | undefined;
      if (trustedDiagnosisOutput !== undefined) {
        validatedPatientReply = validatePatientOutputV1(
          trustedDiagnosisOutput,
          {
            casePackage: context.casePackage,
            safeCaseView,
            completedTests: context.session.completedTests,
            userText: text,
                ...
                ...(attempt === 0
                  ...
                  : {
                        regenerationInstruction:
                          "The previous structured output was rejected by the server gate. Answer the same user message again as the same patient, using patientProfile and the complete committed conversation history to preserve identity and continuity. Never mention missing case data, a profile, a prompt, or a system setting. Creatively fill ordinary non-medical identity and daily-life details when useful, keep every invented detail consistent with the full history, and speak naturally rather than selecting a canned phrase. Cite only supplied IDs for medical facts, explicit persona anchors, completed tests, or test actions; remove unsupported medical claims. When a completed test report contains a diagnosis-like term, the reply must be exactly that deterministic report verbatim with no prefix, suffix, or additional interpretation. A fact-free attitude, emotion, acknowledgement, or hypothetical test concern that neither orders a test nor asks for an existing result must use interactionKind social_chat. Reanalyze diagnosisIntent only from this userText: submit only when the player clearly commits to one primary diagnosis; treat equally asserted multiple diseases without a clear primary as continue_dialogue and copy those exact terms into candidateDiagnoses; copy every diagnosis term exactly from userText and never judge correctness. Do not reveal a diagnosis, an uncompleted test result, or any new medical fact in reply.",
                    }),
              }),
            );
      ...
      const payload: TurnBufferPayload = {
        text,
        ...(trustedDiagnosisOutput === undefined
          ? {}
          : { committedText: COMMITTED_DIAGNOSIS_TURN_TEXT }),
        action,
    ...
    if (operation.kind === "turn") {
      ...
      if (!turnAlreadyCommitted) {
        session.turns.push({
          ...
          text: payload.committedText ?? payload.text,
