...
function assertSafeViewMatchesCase(
  ...
  const expectedTests = buildSafePatientCaseView(
    casePackage,
    context.completedTests ?? [],
  ).tests;
  if (!structurallyEqual(safeCaseView.tests, expectedTests)) {
    reject("Patient output gate received an invalid safe case view for tests.");
  }
}
...
export function validatePatientOutputV1(
  ...
  const authorizedCompletedReports = [...completedIds]
    .filter((testId) => testStatusById.get(testId) === "completed")
    .flatMap((testId) => {
      const report = context.safeCaseView.tests.find(
  ...
  if (
    diagnosisTerms.some((term) => normalizedReply.includes(term)) &&
    !authorizedCompletedReports.includes(reply)
  ) {
