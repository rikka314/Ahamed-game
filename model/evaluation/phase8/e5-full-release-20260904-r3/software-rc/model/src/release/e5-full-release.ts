...
export const E5_FULL_ACCEPTANCE_REPORT_SCHEMA_VERSION =
  "e5-full-acceptance-report-v2" as const;
export const E5_RUNTIME_RELEASE_MANIFEST_SCHEMA_VERSION =
  "e5-runtime-release-manifest-v2" as const;
...
export const E5_REQUIRED_LOCAL_COMMAND_NAMES = [
  ...
  "model.cases:validate:dialogue",
  "model.eval:phase7:offline",
  "model.eval:case-ai-verify",
  "model.eval:c7-dialogue-verify",
  "model.eval:e3:persona-verify",
  "model.eval:e4-closure-verify",
  "game.lint",
  "game.typecheck",
  "game.test",
  "game.test:contract",
...
export interface E5AcceptanceReportV1 {
  ...
  observedQualityFindings: E5Finding[];
  findings: E5Finding[];
}
...
export interface E5ProviderObservation {
  ...
  qualityStatus?: E5ObservationStatus;
  providerName?: string;
...
export function buildE5AcceptanceReport(input: {
  ...
  observedQualityFindings?: readonly E5Finding[];
  generatedAt: string;
  ...
  for (const binding of input.evidenceBindings) {
    ...
    normalizeArtifactPath(binding.path);
  ...
  for (const [index, finding] of (input.observedQualityFindings ?? []).entries()) {
    assertFinding(finding, `observedQualityFindings[${index}]`);
  ...
  return {
    ...
    observedQualityFindings: structuredClone([...(input.observedQualityFindings ?? [])]),
    findings,
  };
}
...
function assertAcceptanceReport(value: unknown): asserts value is E5AcceptanceReportV1 {
  ...
  if (
    ...
    !Array.isArray(value.observedQualityFindings)
  ) {
  ...
  const reconstructed = buildE5AcceptanceReport({
    ...
    observedQualityFindings: value.observedQualityFindings as unknown as E5Finding[],
...
function normalizeArtifactPath(path: string): string {
  if (
    ...
    throw new Error("E5 release artifact path must be a portable relative path");
  }
  const segments = path.split("/");
  if (segments.some((segment) => segment === "" || segment === "." || segment === "..")) {
    throw new Error("E5 release artifact path contains an unsafe segment");
  }
...
function assertProviderObservation(observation: E5ProviderObservation): void {
  ...
  if (
    ...
    (!nonEmptyString(observation.providerName) ||
      ...
      observation.qualityStatus === undefined ||
      !["passed", "failed", "not_run", "incomplete", "stale"].includes(
...
function assertProviderBinding(
  acceptanceProvider: E5ObservationStatus,
  observation: E5ProviderObservation,
): void {
  const expectedAcceptanceStatus: E5ObservationStatus = observation.status === "observed"
    ? observation.qualityStatus!
    : observation.status;
  if (acceptanceProvider !== expectedAcceptanceStatus) {
    throw new Error("E5 acceptance provider status does not match provider observation");
  }
}

function assertEvidenceBindingsInArtifacts(
  acceptance: E5AcceptanceReportV1,
  artifacts: readonly E5ReleaseArtifactBinding[],
): void {
  const artifactByPath = new Map(artifacts.map((artifact) => [artifact.path, artifact]));
  for (const binding of acceptance.evidenceBindings) {
    const path = normalizeArtifactPath(binding.path);
    const artifact = artifactByPath.get(path);
    if (binding.status === "missing") {
      if (artifact !== undefined) {
        throw new Error(`E5 missing evidence binding unexpectedly exists in artifacts: ${binding.name}`);
      }
      continue;
    }
    if (artifact === undefined || artifact.sha256 !== binding.sha256) {
      throw new Error(`E5 evidence binding is not backed by a matching artifact: ${binding.name}`);
    }
  }
}
...
export function buildE5RuntimeReleaseManifest(input: {
  ...
  assertProviderBinding(acceptanceValue.provider, input.providerObservation);
  if (
  ...
  assertEvidenceBindingsInArtifacts(acceptanceValue, artifacts);
...
export function verifyE5RuntimeReleaseManifest(
  ...
  assertAcceptanceReport(acceptanceValue);
  assertProviderBinding(acceptanceValue.provider, manifest.providerObservation);
  ...
  assertEvidenceBindingsInArtifacts(acceptanceValue, manifest.artifacts);
