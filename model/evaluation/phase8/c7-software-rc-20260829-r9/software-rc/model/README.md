...
> 当前基线（2026-08-29）：对话架构重构 C0–C7 已完成。旧 Phase 8 Software RC、manifest 与测试证据继续标记为 `superseded`；新证据链使用 5 个重新双 AI 验证的病例、165 条安全语料双角色 AI 验证，以及五病例三人格 60 轮真实 Patient Agent 基准。新的 Software RC 保持远程交互关闭，并在全新临时目录通过 `model/ + share/` 安装、构建、全量测试和 deterministic CLI smoke。
...
Anthropic adapter 固定到官方 `https://api.anthropic.com`，忽略 `ANTHROPIC_BASE_URL`，并关闭 SDK 自动重试；在线问诊使用与 OpenAI 相同的 `v0.2.0` Patient Agent 输入输出契约，最终评分仍使用 Review。它保持为离线验证通过的可选 adapter，不会读取或复用当前第三方 `MODEL_API_KEY`，也不会进入当前 MVP session。若未来重新启用真实 C01，必须另行批准并配置独立 `ANTHROPIC_API_KEY`。
...
- `src/cli/command-parser.ts` 固定 `/cases`、`/start`、`/resume`、`/status`、问诊、检查、鉴别诊断、诊断、结果、取消、帮助和退出命令；普通自然语言先进入 Patient Agent，只有经过门禁的结构化 `requestedTestId` 才能在同一回合触发检查，仍不会隐式提交诊断。
...
- Patient Agent 每轮接收完整 `PatientProfile`、`SafePatientCaseView`、最近 12 个已提交轮次、已披露事实、完成检查和连续离题轮数；永远不接收 target diagnosis、answer key、rubric、隐藏事实或尚未完成的检查报告。`v0.2.0` 输出必须声明 interaction kind 与引用 ID，经过精确字段、事实/人格/已完成检查白名单、诊断/未完成检查泄漏门和安全源文本锚点后，模型回复才会直接提交给 CLI；服务端不再用本地模板覆盖。
...
- 当前对话重构：**C0–C7 已完成**。C6 证据为 `evaluation/phase8/c6-cli-acceptance-20260829-r6.json`；C7 本地验收为 `evaluation/phase8/c7-runtime-acceptance-20260829-r10.json`，真实对话基准为 `evaluation/phase8/c7-dialogue-live-20260829-r5/`。新 Software RC 只证明模型层离线候选已重建；目标服务器 Phase 8-B 与远程交互启用仍是后续独立发布门。
