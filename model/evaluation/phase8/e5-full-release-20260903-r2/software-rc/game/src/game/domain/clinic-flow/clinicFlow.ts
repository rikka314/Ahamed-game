...
export type ClinicFlowState = {
  ...
  seatOccupantNpcId: NpcIdV1 | null;
  completedSessionIds: SessionIdV1[];
  completedPatientRoleIds: PatientRoleIdV1[];
...
export function transitionClinicFlow(
  ...
  switch (command.type) {
    ...
    case "shift.start": {
      if (state.phase !== "computer_opened" && state.phase !== "shift_completed") {
      ...
      const duplicateSessionIds = findDuplicates(
        command.patients.map(({ sessionId }) => sessionId),
      );
      const duplicatePatientRoleIds = findDuplicates(
        command.patients.map(({ patientRoleId }) => patientRoleId),
      );

      if (duplicateNpcIds.length > 0 || duplicateQueueEntryIds.length > 0) {
        return rejected(state, "queue_ids_must_be_unique");
      }
      if (duplicateSessionIds.length > 0 || duplicatePatientRoleIds.length > 0) {
        return rejected(state, "patient_bindings_must_be_unique");
      }
      if (
        command.patients.some(
          ({ sessionId, patientRoleId }) =>
            state.completedSessionIds.includes(sessionId) ||
            state.completedPatientRoleIds.includes(patientRoleId),
        )
      ) {
        return rejected(state, "patient_binding_already_completed");
      }
    ...
    case "patient.departure.start": {
      if (
        ...
        state.currentPatient.sessionId !== command.sessionId ||
    ...
    case "patient.departure.complete": {
      ...
      return applied(state, command.commandId, {
        ...
        completedSessionIds: [
          ...
          state.currentPatient.sessionId,
        ],
        completedPatientRoleIds: [
          ...state.completedPatientRoleIds,
          state.currentPatient.patientRoleId,
        ],
      });
    }
  }
