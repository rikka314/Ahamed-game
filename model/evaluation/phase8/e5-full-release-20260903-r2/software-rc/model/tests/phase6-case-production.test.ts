...
import { migrateCasePackageV1ToV2 } from "../src/domain/case-package.js";
import type {
  AiCaseCrossReviewV3,
  CasePackage,
...
test("Phase 6 publication requires two independent approved AI validations and no human signature", () => {
  const bundle = loadPhase6CaseBundles()[0]!;
...
test("Phase 6 publication lock prevents reentrant writers from mixing package and sidecar", () => {
  const firstBundle = loadPhase6CaseBundles()[0]!;
...
test("Phase 6 validation reports content, minimum-data, and trajectory defects", () => {
  const baseline = loadPhase6CaseBundles()[0]!;
...
test("Phase 6 validates CasePackage v2 through the supported-package contract", () => {
  const bundle = structuredClone(loadPhase6CaseBundles()[0]!);
  assert.equal(bundle.casePackage.schemaVersion, "case-package-v1-rc1");
  if (bundle.casePackage.schemaVersion !== "case-package-v1-rc1") return;
  const migrated = migrateCasePackageV1ToV2(bundle.casePackage, {
    patientRoleId: "patient-role.phase6-v2",
    ...
    provenanceSources: [{
      sourceId: "source.phase6.synthetic",
      sourceRole: "synthetic_structure",
      title: "Phase 6 v2 regression fixture",
      authorsOrOrganization: "AhaMed tests",
      ...
      projectUsage: "Automated regression only.",
      includesVerbatimExcerpt: false,
      verifiedCaseFields: ["patientFacts"],
      licenseAssessment: "cleared",
      riskNotes: [],
    }],
  });
  bundle.casePackage = migrated;
  bundle.trajectories.caseVersion = migrated.caseVersion;
