...
test("controller and patient calls use strict schemas and minimum role data", async () => {
  ...
  assert.ok(patientSchema.required.includes("diagnosisIntent"));
  assert.deepEqual(patientSchema.properties["diagnosisIntent"], {
    type: "object",
...
test("OpenAI Patient adapter carries Persona v2 identity and modifiers without hidden case data", async () => {
