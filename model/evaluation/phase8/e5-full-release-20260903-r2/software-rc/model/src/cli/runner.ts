...
const CLI_HELP_ZH = `命令：
  ...
  直接输入中文                   与虚构患者对话；AI 识别明确的最终主诊断后自动评分
  /tests                         列出公共检查目录
  /test <testId>                 申请检查
...
export function createCliProvider(
  ...
  return createLocalDevOpenAIProvider(config, environment);
}
...
export async function runCli(options: RunCliOptions): Promise<CliRunResult> {
  ...
  options.io.write("AhaMed Doctor Game AI Patient CLI");
  options.io.write(
      ...
      try {
        ...
        switch (command.kind) {
          ...
          case "ask": {
            ...
            const internalTurn = await service.askPatient({
              sessionId: selected.session.sessionId,
              clientTurnId: nextRequestId("turn"),
              text: command.text,
            });
            if (internalTurn.diagnosisSubmission !== undefined) {
              const submissionDifferentials = [
                ...new Set([
                  ...differentials,
                  ...internalTurn.diagnosisSubmission.differentials,
                ]),
              ];
              const result = toEvaluationResultV1(
                await service.submitDiagnosis({
                  ...
                  primaryDiagnosis:
                    internalTurn.diagnosisSubmission.primaryDiagnosis,
                  differentials: submissionDifferentials,
                }),
              );
              current = toCreateSessionResponseV1(
                service.getSessionSnapshot(
            ...
            const turn = toTurnCompletedV1(internalTurn);
