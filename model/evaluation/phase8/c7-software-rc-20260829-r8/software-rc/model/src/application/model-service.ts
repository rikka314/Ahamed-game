...
function renderGroundedPatientReply(
  locale: string,
  facts: Array<{ status: "present" | "absent" | "unknown"; value: string }>,
): string {
  if (facts.length === 0) {
...
function renderOtherReply(locale: string, chiefComplaint: string): string {
  const complaint = chiefComplaint.trim();
  if (complaint.length === 0) {
    return locale.startsWith("zh")
      ? "请询问与当前虚构患者病例相关的症状或病史。"
      : "Please ask about the current fictional patient's symptoms or history.";
  }
  const suffix = /[。！？!?]$/u.test(complaint) ? "" : locale.startsWith("zh") ? "。" : ".";
  return locale.startsWith("zh")
    ? `我主要是${complaint}${suffix}`
    : `My main concern is ${complaint}${suffix}`;
}

function isChiefComplaintQuestion(value: string): boolean {
  const normalized = value
    .normalize("NFKC")
    .toLocaleLowerCase("en-US")
    .replace(/[^\p{L}\p{N}]+/gu, "");
  return (
    /^(?:(?:请)?(?:告诉|告知)我|请问)?(?:你|您|患者|病人|这位患者|这个患者)?(?:现在|主要)?有(?:什么|哪些)(?:症状|不适)(?:吗|么|呢|呀|啊)?$/u.test(
      normalized,
    ) ||
    /^(?:请问)?(?:你|您)?(?:哪里|哪儿)(?:不舒服|难受)(?:吗|么|呢|呀|啊)?$/u.test(normalized) ||
    /^(?:what)(?:symptoms|problems|complaints)(?:do)?(?:you)?(?:have)$/u.test(
      normalized,
    )
  );
}

function renderSocialReply(
  locale: string,
  text: string,
  patientDisplay: {
  ...
  if (!locale.startsWith("zh")) {
    if (/^(?:hello|hi|hey)$/u.test(normalized)) return "Hello, doctor.";
    if (/(?:yourname|whatareyoucalled)/u.test(normalized)) {
      return `My name is ${patientDisplay.patientDisplayName}.`;
    }
    return "I am listening, doctor. Please continue.";
  }
  if (/^(?:你好|您好|嗨|哈喽|hello|hi)$/iu.test(normalized)) {
    return "你好，医生。";
  }
  if (
    /(?:你好).*(?:没|不).*(?:回|理)|(?:没|不).*(?:回|理).*(?:你好)|礼貌/iu.test(
      normalized,
    )
  ) {
    return "不好意思，医生，刚才没及时回应您。你好。";
  }
  if (/(?:你叫什么|你的名字|叫什么名字|怎么称呼你)/u.test(normalized)) {
    return `我叫${patientDisplay.patientDisplayName}。`;
  }
  if (/(?:你多大|你几岁|年龄多大)/u.test(normalized)) {
    return patientDisplay.ageBand === undefined
      ? "这个我不太方便说，医生。"
      : `我是${patientDisplay.ageBand}。`;
  }
  if (/(?:你是男|你是女|男的女的|什么性别)/u.test(normalized)) {
    return patientDisplay.genderDisplay === undefined
      ? "这个和您问诊有关系吗，医生？"
      : `我是${patientDisplay.genderDisplay}。`;
  }
  if (/(?:笨|傻|蠢|没用|废物|滚)/u.test(normalized)) {
    return "医生，您别生气，我会配合回答的。";
  }
  if (/(?:谢谢|多谢|感谢)/u.test(normalized)) {
    return "不客气，医生。";
  }
  return "我在听，医生。您继续问吧。";
}
...
function validateAndRebuildTurnBufferPayload(
  ...
  const expectedAction =
    payload.legacyV3Shape === true && payload.requestedFactIds.length === 0
      ? "ask_patient"
      : payload.requestedFactIds.every((factId) => disclosedBefore.has(factId))
        ? "repeat"
        : "ask_patient";
  let rebuiltReply: string;
  if (payload.action === "other") {
    rebuiltReply = isChiefComplaintQuestion(payload.text)
      ? renderOtherReply(
          casePackage.locale,
          casePackage.playerVisible.chiefComplaint,
        )
      : renderSocialReply(
          casePackage.locale,
          payload.text,
    ...
    rebuiltReply = renderGroundedPatientReply(
      casePackage.locale,
      factsUsed.map((factId) => casePackage.patientFacts[factId]!),
    );
  }

  if (
    (payload.action === "other" &&
      (payload.requestedFactIds.length !== 0 ||
        factsUsed.length !== 0 ||
        payload.disclosedFacts.length !== 0)) ||
    (payload.action !== "other" && payload.action !== expectedAction) ||
    (payload.action !== "other" &&
      payload.requestedFactIds.length === 0 &&
      payload.legacyV3Shape !== true) ||
    (payload.action === "ask_patient" &&
      factsUsed.length === 0 &&
      payload.legacyV3Shape !== true) ||
    !isDeepStrictEqual(payload.disclosedFacts, expectedNewFacts) ||
    payload.response.reply !== rebuiltReply ||
    payload.response.effects.length !== 0
  ) {
    corruptOperation(
      "Validated turn buffer no longer matches the immutable case package.",
    );
  }
...
export class ModelService {
  ...
  private async executeTurnOperation(
    ...
    try {
      ...
      const patientInput = {
        ...
        recentTurns: context.session.turns.slice(-12).map((turn) => ({
          turnId: turn.turnId,
          userText: turn.text,
              ...
              ...(attempt === 0
                ...
                : {
                      regenerationInstruction:
                        "The previous output was rejected by the server gate. Regenerate once using only facts, persona facts, completed test reports, and test IDs present in the supplied safe case view. Every cited fact, persona fact, or completed test must be grounded by including its exact supplied value or report in the reply. When the user asks whether the previously stated symptoms are still present, select a fact ID already present in disclosedFactIds and repeat that exact safe value. When the user directly asks about interests or whether the patient is busy, cite the matching persona.interest.* or persona.daily_life value, or use the exact off-topic reminder. If all reference arrays are empty and there is no test action, reply with exactly one allowlisted phrase and nothing else: 你好医生 / 好的 / 谢谢 / 医生还是继续问我的病情吧 / 我不知道 / 我不清楚 / 我没注意 / 我不确定. For a persona reply use exactly 我平时喜欢 plus one exact supplied persona value. For a suggested test, use the exact display name and the shortest confirmation question required by the Patient Agent prompt. For a requested test, return requestedTestId and reply exactly 好的. Do not reveal a diagnosis, an uncompleted test result, or any new fact.",
                  }),
            }),
          );
  ...
  private requireCase(session: SessionAggregate): CasePackage {
