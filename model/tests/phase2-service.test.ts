...
test("sends the stable patient identity and complete committed conversation history to the Patient Agent every turn", async (t) => {
  const patientInputs: PatientInput[] = [];
  const service = createSqliteModelService({
    databasePath: await createDatabasePath(t),
    cases: [createCaseFixture()],
    provider: createFixtureProvider({
      generatePatientReply: async (input) => {
        patientInputs.push(structuredClone(input));
        return {
          reply: `I am still Taylor Chen; this is turn ${patientInputs.length}.`,
  ...
  const created = await createSession(service, "create-history");

  for (let turn = 1; turn <= 14; turn += 1) {
    await service.askPatient({
      sessionId: created.session.sessionId,
      clientTurnId: `history-turn-${turn}`,
      text: turn % 2 === 0 ? "What is your name?" : `Question ${turn}`,
    });
  }

  assert.equal(patientInputs.length, 14);
  assert.ok(
    patientInputs.every(
      ({ patientProfile }) => patientProfile.displayName === "Test Patient",
    ),
  );
  assert.equal(patientInputs[13]!.recentTurns.length, 13);
  assert.equal(patientInputs[13]!.recentTurns[0]!.userText, "Question 1");
  assert.equal(
    patientInputs[13]!.recentTurns[12]!.patientReply,
    "I am still Taylor Chen; this is turn 13.",
  );
});
