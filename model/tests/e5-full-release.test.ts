...
function buildReport(overrides: {
  ...
  observedQualityFindings?: Parameters<typeof buildE5AcceptanceReport>[0]["observedQualityFindings"];
  ...
  return buildE5AcceptanceReport({
    ...
    ...(overrides.observedQualityFindings === undefined
      ? {}
      : { observedQualityFindings: overrides.observedQualityFindings }),
    generatedAt,
  });
}
...
test("E5 reports 5/30 coverage plus stale and not-run observations as incomplete", () => {
  ...
  assert.equal(report.schemaVersion, "e5-full-acceptance-report-v2");
...
test("E5 preserves non-blocking model quality findings in the final decision", () => {
  const qualityFinding = {
    code: "C7_CASE_AI_REVIEW_REJECTED",
    status: "failed" as const,
    scope: "case-ai-review:case_c08_cardiometabolic_001",
    message: "The independent reviewer rejected this case package.",
    expected: "approved",
    actual: "rejected",
  };
  const report = buildReport({ observedQualityFindings: [qualityFinding] });

  assert.equal(report.decision, "reported_with_failures");
  assert.deepEqual(report.observedQualityFindings, [qualityFinding]);
  assert.ok(report.findings.some(({ code }) => code === qualityFinding.code));
});
...
test("E5 runtime manifest verifies artifacts and rejects artifact, acceptance, and self-hash drift", () => {
  ...
  try {
    ...
    assert.equal(manifest.schemaVersion, "e5-runtime-release-manifest-v2");
    assert.equal("approvedProviders" in manifest, false);
    assert.equal(manifest.providerObservation.status, "observed");
    assert.equal(
      manifest.artifacts.length,
      E5_REQUIRED_EVIDENCE_BINDING_NAMES.length + 2,
    );
    assert.equal(
      verifyE5RuntimeReleaseManifest(manifest, root).artifactCount,
      E5_REQUIRED_EVIDENCE_BINDING_NAMES.length + 2,
    );
    const failedProviderBaseReport = buildReport({ providerStatus: "failed" });
    const failedProviderReport: E5AcceptanceReportV1 = {
      ...failedProviderBaseReport,
      evidenceBindings: failedProviderBaseReport.evidenceBindings.map((binding) => ({
        ...binding,
        sha256: createHash("sha256")
          .update(`${binding.name}\n`)
          .digest("hex"),
    ...
    writeFileSync(
      join(root, "evidence", "acceptance.json"),
      `${JSON.stringify(failedProviderReport, null, 2)}\n`,
    );
    const observedButRejectedManifest = buildE5RuntimeReleaseManifest({
      rootDirectory: root,
      artifactPaths: [
        "evidence/quality.json",
        ...E5_REQUIRED_EVIDENCE_BINDING_NAMES.map((name) => `evidence/${name}.json`),
      ...
      providerObservation: {
        ...
        configuredModelId: "test-model",
        actualModelId: "test-model",
        findings: ["Provider identity observed; quality review did not approve."],
      },
      qualityFindings: failedProviderReport.findings,
      generatedAt,
    });
    assert.equal(observedButRejectedManifest.providerObservation.status, "observed");
    assert.equal(observedButRejectedManifest.providerObservation.qualityStatus, "failed");
    assert.equal(
      verifyE5RuntimeReleaseManifest(observedButRejectedManifest, root).artifactCount,
      E5_REQUIRED_EVIDENCE_BINDING_NAMES.length + 2,
    );
    writeFileSync(
      join(root, "evidence", "acceptance.json"),
...
test("E5 runtime artifact collection excludes generated compiler metadata", () => {
  const root = mkdtempSync(join(tmpdir(), "ahamed-e5-build-cache-"));
  try {
    for (const directory of ["model", "share", "game"]) {
      mkdirSync(join(root, directory));
    }
    writeFileSync(join(root, "game", "runtime.ts"), "export const runtime = true;\n");
    writeFileSync(join(root, "game", "tsconfig.tsbuildinfo"), "generated cache\n");
    writeFileSync(join(root, "game", "next-env.d.ts"), "generated Next.js references\n");

    const paths = collectE5RuntimeArtifactPaths(root);
    assert.ok(paths.includes("game/runtime.ts"));
    assert.equal(paths.some((path) => path.endsWith(".tsbuildinfo")), false);
    assert.equal(paths.includes("game/next-env.d.ts"), false);
  } finally {
    rmSync(root, { recursive: true, force: true });
  }
});
