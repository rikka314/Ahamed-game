...
test("one-case, five-case, and thirty-case manifests use the same semantic validator", () => {
  for (const caseCount of [1, 5, 30]) {
    ...
    assert.deepEqual(report.findings, [], `${caseCount}-case findings`);
...
test("manifest dry-run reports identity, quota, and red-flag policy drift as structured findings", () => {
  ...
  manifest.releasePolicy.requiredPersonas[0]!.count = 4;
  manifest.releasePolicy.diseaseDomainQuotas[0]!.count = 4;
  manifest.releasePolicy.difficultyQuotas.basic = 4;
...
test("artifact inspection reports missing files and canonical hash drift without scanning extra drafts", () => {
  ...
  try {
    ...
    for (const path of [selected.path, selected.regressionPath]) {
...
test("the committed five-case compatibility manifest is structurally valid and explicitly bound", () => {
  const manifest = JSON.parse(
    readFileSync("cases/manifest.phase6-compat.v2-rc1.json", "utf8"),
  ) as unknown;
  const report = validateCaseManifestV2(manifest, {
    knownRedFlagPolicyVersions: ["red-flag-policy-manifest-v2"],
  });
  ...
  assert.equal(report.metrics.caseCount, 5);
});
