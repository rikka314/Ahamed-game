...
const PATIENT_TEXTURES: Record<string, { standing: string; seated: string }> = {
  "npc.patient.graybox-01": {
    standing: "patient-placeholder-01",
    seated: "patient-seated-placeholder-01",
  },
  "npc.patient.graybox-02": {
    standing: "patient-placeholder-02",
    seated: "patient-seated-placeholder-02",
  },
};
...
export class WorldScene extends Phaser.Scene {
  ...
  private createActors(): void {
    ...
    GRAYBOX_PATIENTS.forEach((patient) => {
      const queueAnchor = this.requireMapObject(patient.queueAnchorId);
      const textures = this.requirePatientTextures(patient.npcId);
      ...
      this.patientSprites.set(patient.npcId, sprite);
    });
  ...
  private configureBridge(): void {
    this.stopListening = [
      ...
      gameCommands.on("clinic.start-shift", ({ commandId, shiftId }) => {
        this.startShift(commandId, shiftId);
      }),
      gameCommands.on("clinic.dismiss-current", ({ commandId }) => {
        this.dismissCurrentPatient(commandId);
      }),
  ...
  private startShift(commandId: string, shiftId: string): void {
  ...
  private showWaitingQueue(): void {
    this.clinicFlow.queue.forEach((patient) => {
      ...
      const sprite = this.patientSprites.get(patient.npcId);
      if (!sprite) {
        return;
      }
      sprite.setTexture(this.requirePatientTextures(patient.npcId).standing);
      sprite.enableBody(true, anchor.x, anchor.y, true, true);
      this.emitPatientVisual(patient.npcId, "standing", true);
    });
  }
  ...
  private animatePatientEntry(patient: PatientQueueEntry): void {
    ...
    this.moveAlongAnchors(
      ...
      () => {
        ...
        if (seated) {
          sprite.setTexture(this.requirePatientTextures(patient.npcId).seated);
          sprite.setBodySize(8, 4).setOffset(2, 7);
          this.emitPatientVisual(patient.npcId, "seated", true);
          worldEvents.emit("speech.show", {
  ...
  private dismissCurrentPatient(commandId: string): void {
    ...
    worldEvents.emit("speech.show", {
      messageId: `speech.goodbye.${patient.npcId}`,
      speakerId: patient.npcId,
      speakerRole: "patient",
      text: "谢谢医生，我先离开了。",
    });
    this.moveAlongAnchors(
      ...
      () => {
        sprite.disableBody(true, true);
        this.emitPatientVisual(patient.npcId, "standing", false);
        this.applyClinicCommand({
          type: "patient.departure.complete",
          commandId: `departure-complete.${patient.npcId}`,
          npcId: patient.npcId,
        });
  ...
  private emitClinicFlow(): void {
    const snapshot: ClinicFlowSnapshot = {
      ...
      completedCount: this.clinicFlow.completedNpcIds.length,
    };
  ...
  private emitPatientVisual(
    ...
    const visible = Boolean(
      sprite && spriteVisible && this.cameras.main.worldView.contains(sprite.x, sprite.y),
    );
    worldEvents.emit("patient.visual.updated", { npcId, pose, visible });
  }

  private requirePatientTextures(npcId: string): { standing: string; seated: string } {
    const textures = PATIENT_TEXTURES[npcId];
    if (!textures) {
      throw new Error(`Patient textures are missing: ${npcId}`);
    }
    return textures;
  }
